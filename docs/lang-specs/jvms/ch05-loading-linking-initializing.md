...
[5.1. The Run-Time Constant Pool](ch05-loading-linking-initializing.md#jvms-5.1)

[5.2. Java Virtual Machine Startup](ch05-loading-linking-initializing.md#jvms-5.2)

[5.3. Creation and Loading](ch05-loading-linking-initializing.md#jvms-5.3)

[5.3.1. Loading Using the Bootstrap Class Loader](ch05-loading-linking-initializing.md#jvms-5.3.1)

[5.3.2. Loading Using a User-defined Class Loader](ch05-loading-linking-initializing.md#jvms-5.3.2)

[5.3.3. Creating Array Classes](ch05-loading-linking-initializing.md#jvms-5.3.3)

[5.3.4. Loading Constraints](ch05-loading-linking-initializing.md#jvms-5.3.4)

[5.3.5. Deriving a Class from a `class` File Representation](ch05-loading-linking-initializing.md#jvms-5.3.5)

[5.3.6. Modules and Layers](ch05-loading-linking-initializing.md#jvms-5.3.6)

[5.4. Linking](ch05-loading-linking-initializing.md#jvms-5.4)

[5.4.1. Verification](ch05-loading-linking-initializing.md#jvms-5.4.1)

[5.4.2. Preparation](ch05-loading-linking-initializing.md#jvms-5.4.2)

[5.4.3. Resolution](ch05-loading-linking-initializing.md#jvms-5.4.3)

[5.4.3.1. Class and Interface Resolution](ch05-loading-linking-initializing.md#jvms-5.4.3.1)

[5.4.3.2. Field Resolution](ch05-loading-linking-initializing.md#jvms-5.4.3.2)

[5.4.3.3. Method Resolution](ch05-loading-linking-initializing.md#jvms-5.4.3.3)

[5.4.3.4. Interface Method Resolution](ch05-loading-linking-initializing.md#jvms-5.4.3.4)

[5.4.3.5. Method Type and Method Handle Resolution](ch05-loading-linking-initializing.md#jvms-5.4.3.5)

[5.4.3.6. Dynamically-Computed Constant and Call Site Resolution](ch05-loading-linking-initializing.md#jvms-5.4.3.6)

[5.4.4. Access Control](ch05-loading-linking-initializing.md#jvms-5.4.4)

[5.4.5. Method Overriding](ch05-loading-linking-initializing.md#jvms-5.4.5)

[5.4.6. Method Selection](ch05-loading-linking-initializing.md#jvms-5.4.6)

[5.5. Initialization](ch05-loading-linking-initializing.md#jvms-5.5)

[5.6. Binding Native Method Implementations](ch05-loading-linking-initializing.md#jvms-5.6)

[5.7. Java Virtual Machine Termination](ch05-loading-linking-initializing.md#jvms-5.7)


The Java Virtual Machine dynamically loads, links and initializes classes and interfaces. Loading is the process of finding the binary representation of a class or interface type with a particular name and *creating* a class or interface from that binary representation. Linking is the process of taking a class or interface and combining it into the run-time state of the Java Virtual Machine so that it can be executed. Initialization of a class or interface consists of executing the class or interface initialization method `<clinit>` ([§2.9.2](ch02-jvm-structure.md#jvms-2.9.2)).

In this chapter, [§5.1](ch05-loading-linking-initializing.md#jvms-5.1) describes how the Java Virtual Machine derives symbolic references from the binary representation of a class or interface. [§5.2](ch05-loading-linking-initializing.md#jvms-5.2) explains how the processes of loading, linking, and initialization are first initiated by the Java Virtual Machine. [§5.3](ch05-loading-linking-initializing.md#jvms-5.3) specifies how binary representations of classes and interfaces are loaded by class loaders and how classes and interfaces are created. Linking is described in [§5.4](ch05-loading-linking-initializing.md#jvms-5.4). [§5.5](ch05-loading-linking-initializing.md#jvms-5.5) details how classes and interfaces are initialized. [§5.6](ch05-loading-linking-initializing.md#jvms-5.6) introduces the notion of binding native methods. Finally, [§5.7](ch05-loading-linking-initializing.md#jvms-5.7) describes when the Java Virtual Machine terminates.


## 5.1. The Run-Time Constant Pool


The Java Virtual Machine maintains a run-time constant pool for each class and interface ([§2.5.5](ch02-jvm-structure.md#jvms-2.5.5)). This data structure serves many of the purposes of the symbol table of a conventional programming language implementation. The `constant_pool` table in the binary representation of a class or interface ([§4.4](ch04-class-file-format.md#jvms-4.4)) is used to construct the run-time constant pool upon class or interface creation ([§5.3](ch05-loading-linking-initializing.md#jvms-5.3)).

There are two kinds of entry in the run-time constant pool: symbolic references, which may later be resolved ([§5.4.3](ch05-loading-linking-initializing.md#jvms-5.4.3)), and static constants, which require no further processing.
...
- A symbolic reference to a class or interface is derived from a `CONSTANT_Class_info` structure ([§4.4.1](ch04-class-file-format.md#jvms-4.4.1)). Such a reference gives the name of the class or interface in the following form:


  - For a nonarray class or an interface, the name is the binary name ([§4.2.1](ch04-class-file-format.md#jvms-4.2.1)) of the class or interface.
  ...
  - For an array class of *n* dimensions, the name begins with *n* occurrences of the ASCII `[` character followed by a representation of the element type:
    ...
    - If the element type is a primitive type, it is represented by the corresponding field descriptor ([§4.3.2](ch04-class-file-format.md#jvms-4.3.2)).
...
- A symbolic reference to a field of a class or an interface is derived from a `CONSTANT_Fieldref_info` structure ([§4.4.2](ch04-class-file-format.md#jvms-4.4.2)). Such a reference gives the name and descriptor of the field, as well as a symbolic reference to the class or interface in which the field is to be found.

- A symbolic reference to a method of a class is derived from a `CONSTANT_Methodref_info` structure ([§4.4.2](ch04-class-file-format.md#jvms-4.4.2)). Such a reference gives the name and descriptor of the method, as well as a symbolic reference to the class in which the method is to be found.

- A symbolic reference to a method of an interface is derived from a `CONSTANT_InterfaceMethodref_info` structure ([§4.4.2](ch04-class-file-format.md#jvms-4.4.2)). Such a reference gives the name and descriptor of the interface method, as well as a symbolic reference to the interface in which the method is to be found.

- A symbolic reference to a method handle is derived from a `CONSTANT_MethodHandle_info` structure ([§4.4.8](ch04-class-file-format.md#jvms-4.4.8)). Such a reference gives a symbolic reference to a field of a class or interface, or a method of a class, or a method of an interface, depending on the kind of the method handle.

- A symbolic reference to a method type is derived from a `CONSTANT_MethodType_info` structure ([§4.4.9](ch04-class-file-format.md#jvms-4.4.9)). Such a reference gives a method descriptor ([§4.3.3](ch04-class-file-format.md#jvms-4.3.3)).

- A symbolic reference to a *dynamically-computed constant* is derived from a `CONSTANT_Dynamic_info` structure ([§4.4.10](ch04-class-file-format.md#jvms-4.4.10)). Such a reference gives:
...
- A symbolic reference to a *dynamically-computed call site* is derived from a `CONSTANT_InvokeDynamic_info` structure ([§4.4.10](ch04-class-file-format.md#jvms-4.4.10)). Such a reference gives:


  - a symbolic reference to a method handle, which will be invoked in the course of an *invokedynamic* instruction ([§invokedynamic](ch06-instruction-set.md#jvms-6.5.invokedynamic)) to compute an instance of `java.lang.invoke.CallSite`;
...
- A string constant is a `reference` to an instance of class `String`, and is derived from a `CONSTANT_String_info` structure ([§4.4.3](ch04-class-file-format.md#jvms-4.4.3)). To derive a string constant, the Java Virtual Machine examines the sequence of code points given by the `CONSTANT_String_info` structure:
...
- Numeric constants are derived from `CONSTANT_Integer_info`, `CONSTANT_Float_info`, `CONSTANT_Long_info`, and `CONSTANT_Double_info` structures ([§4.4.4](ch04-class-file-format.md#jvms-4.4.4), [§4.4.5](ch04-class-file-format.md#jvms-4.4.5)).
...
- They may be pushed onto the stack by the *ldc* family of instructions ([§ldc](ch06-instruction-set.md#jvms-6.5.ldc), [§ldc_w](ch06-instruction-set.md#jvms-6.5.ldc_w), [§ldc2_w](ch06-instruction-set.md#jvms-6.5.ldc2_w)).

- They may be static arguments to bootstrap methods for dynamically-computed constants and call sites ([§5.4.3.6](ch05-loading-linking-initializing.md#jvms-5.4.3.6)).


An entry in the run-time constant pool is loadable if it is derived from an entry in the `constant_pool` table that is loadable (see [Table 4.4-C](ch04-class-file-format.md#jvms-4.4-310)). Accordingly, the following entries in the run-time constant pool are loadable:
...
The Java Virtual Machine starts up by creating an initial class or interface using the bootstrap class loader ([§5.3.1](ch05-loading-linking-initializing.md#jvms-5.3.1)) or a user-defined class loader ([§5.3.2](ch05-loading-linking-initializing.md#jvms-5.3.2)). The Java Virtual Machine then links the initial class or interface, initializes it, creates an instance of it (if necessary), and invokes a `main` method as specified in (JLS §12.1.4). The invocation of this method drives all further execution. Execution of the Java Virtual Machine instructions of the instance initialization method (if needed) and `main` method of the initial class or interface may cause linking (and consequently creation) of additional classes and interfaces, as well as invocation of additional methods.
...
Creation of a class or interface C denoted by the name `N` consists of the construction of an implementation-specific internal representation of C in the method area of the Java Virtual Machine ([§2.5.4](ch02-jvm-structure.md#jvms-2.5.4)).

Class or interface creation is triggered by another class or interface D, whose run-time constant pool symbolically references C by means of the name `N` ([§5.4.3.1](ch05-loading-linking-initializing.md#jvms-5.4.3.1)). If `N` does not denote an array class, then the Java Virtual Machine relies on a *class loader* to locate a binary representation for a class or interface called `N` ([§4.1](ch04-class-file-format.md#jvms-4.1)). Once a class loader has located a binary representation, it relies in turn on the Java Virtual Machine to derive the class or interface C from the binary representation, and then to create C in the method area. Array classes do not have an external binary representation; they are created by the Java Virtual Machine via a different process.

Class or interface creation may also be triggered by D invoking methods in certain Java SE Platform class libraries ([§2.12](ch02-jvm-structure.md#jvms-2.12)) such as reflection.
...
Due to class loader delegation, the loader `L`<sub>`1`</sub> that initiates loading at the Java Virtual Machine's request may not be the same as the loader `L`<sub>`2`</sub> that completes loading by defining the class or interface. In this case, we say that each of `L`<sub>`1`</sub> and `L`<sub>`2`</sub> *initiates* loading of C, or, equivalently, that each of `L`<sub>`1`</sub> and `L`<sub>`2`</sub> is an *initiating loader* of C. Any loaders in a delegation chain between `L`<sub>`1`</sub> and `L`<sub>`2`</sub> are not considered to be initiating loaders of C.

We will sometimes represent a class or interface using the following notation, instead of using an identifier like C or D:


- `<``N`, `L`<sub>`d`</sub>`>` - where `N` denotes the name of the class or interface and `L`<sub>`d`</sub> denotes the defining loader of the class or interface.

- `N`<sup>`L`<sub>`i`</sub></sup> - where `N` denotes the name of the class or interface and `L`<sub>`i`</sub> denotes an initiating loader of the class or interface.
...
- If `N` denotes either a nonarray class or an interface, and D was defined by the bootstrap class loader, then the bootstrap class loader initiates loading of C ([§5.3.1](ch05-loading-linking-initializing.md#jvms-5.3.1)).

- If `N` denotes either a nonarray class or an interface, and D was defined by a user-defined class loader, then that same user-defined class loader initiates loading of C ([§5.3.2](ch05-loading-linking-initializing.md#jvms-5.3.2)).

- If `N` denotes an array class, then the Java Virtual Machine creates an array class C denoted by `N`, in association with the defining loader of D ([§5.3.3](ch05-loading-linking-initializing.md#jvms-5.3.3)).
...
- If a class loader `L`<sub>`1`</sub> delegates loading of a class C to another loader `L`<sub>`2`</sub>, then for any type T that occurs as the direct superclass or a direct superinterface of C, or as the type of a field in C, or as the type of a formal parameter of a method or constructor in C, or as a return type of a method in C, `L`<sub>`1`</sub> and `L`<sub>`2`</sub> should return the same `Class` object.
...
1.  The class loader `L` can load C directly. This is accomplished by obtaining an array of bytes that purports to represent C as a `ClassFile` structure ([§4.1](ch04-class-file-format.md#jvms-4.1)), and then invoking the method `defineClass` of class `ClassLoader`. Invoking `defineClass` causes the Java Virtual Machine to derive a class or interface C denoted by `N` from the array of bytes using `L`, and then to create C, via the algorithm of [§5.3.5](ch05-loading-linking-initializing.md#jvms-5.3.5). `L` should use the result of `defineClass` as the result of `loadClass`.
...
- Otherwise, the result is the created class or interface C. The Java Virtual Machine records that `L` is an initiating loader of C ([§5.3.4](ch05-loading-linking-initializing.md#jvms-5.3.4)). The process of loading and creating C succeeds.
...
1.  If the component type is a `reference` type, the algorithm of this section ([§5.3](ch05-loading-linking-initializing.md#jvms-5.3)) is applied recursively using `L` in order to load and thereby create the component type of C.
...
2.  The Java Virtual Machine creates a new array class with the indicated component type and number of dimensions.
    ...
    In any case, the Java Virtual Machine then records that `L` is an initiating loader for C ([§5.3.4](ch05-loading-linking-initializing.md#jvms-5.3.4)).

    If the component type is a `reference` type, the accessibility of the array class is determined by the accessibility of its component type ([§5.4.4](ch05-loading-linking-initializing.md#jvms-5.4.4)). Otherwise, the array class is accessible to all classes and interfaces.
...
When a class or interface C = `<``N`<sub>`1`</sub>, `L`<sub>`1`</sub>`>` makes a symbolic reference to a field or method of another class or interface D = `<``N`<sub>`2`</sub>, `L`<sub>`2`</sub>`>`, the symbolic reference includes a descriptor specifying the type of the field, or the return and argument types of the method. It is essential that any class or interface name `N` mentioned in the field or method descriptor ([§4.3.2](ch04-class-file-format.md#jvms-4.3.2), [§4.3.3](ch04-class-file-format.md#jvms-4.3.3)) denote the same class or interface when loaded by `L`<sub>`1`</sub> and when loaded by `L`<sub>`2`</sub>.

To ensure this, the Java Virtual Machine imposes *loading constraints* of the form `N`<sup>`L`<sub>`1`</sub></sup> = `N`<sup>`L`<sub>`2`</sub></sup> during preparation ([§5.4.2](ch05-loading-linking-initializing.md#jvms-5.4.2)) and resolution ([§5.4.3](ch05-loading-linking-initializing.md#jvms-5.4.3)). To enforce these constraints, the Java Virtual Machine will, at certain prescribed times (see [§5.3.1](ch05-loading-linking-initializing.md#jvms-5.3.1), [§5.3.2](ch05-loading-linking-initializing.md#jvms-5.3.2), [§5.3.3](ch05-loading-linking-initializing.md#jvms-5.3.3), and [§5.3.5](ch05-loading-linking-initializing.md#jvms-5.3.5)), record that a particular loader is an initiating loader of a particular class. After recording that a loader is an initiating loader of a class, the Java Virtual Machine must immediately check to see if any loading constraints are violated. If so, the record is retracted, the Java Virtual Machine throws a `LinkageError`, and the loading operation that caused the recording to take place fails.

Similarly, after imposing a loading constraint (see [§5.4.2](ch05-loading-linking-initializing.md#jvms-5.4.2), [§5.4.3.2](ch05-loading-linking-initializing.md#jvms-5.4.3.2), [§5.4.3.3](ch05-loading-linking-initializing.md#jvms-5.4.3.3), and [§5.4.3.4](ch05-loading-linking-initializing.md#jvms-5.4.3.4)), the Java Virtual Machine must immediately check to see if any loading constraints are violated. If so, the newly imposed loading constraint is retracted, the Java Virtual Machine throws a `LinkageError`, and the operation that caused the constraint to be imposed (either resolution or preparation, as the case may be) fails.
...
- The equivalence relation defined by the (reflexive, transitive closure of the) set of imposed constraints implies `N`<sup>`L`</sup> = `N`<sup>`L`'</sup>.
...
Class loaders require the cooperation of the Java Virtual Machine to derive and create a class or interface from a binary representation provided by the loader ([§5.3.1](ch05-loading-linking-initializing.md#jvms-5.3.1), [§5.3.2](ch05-loading-linking-initializing.md#jvms-5.3.2)). The following steps are used to derive a class or interface C denoted by `N` from a purported representation in `class` file format when requested by a class loader `L`.
...
2.  Next, the Java Virtual Machine attempts to parse the purported representation. The purported representation may not in fact be a valid representation of C, so derivation must detect the following problems:
    ...
    - If the purported representation is not a `ClassFile` structure ([§4.1](ch04-class-file-format.md#jvms-4.1), [§4.8](ch04-class-file-format.md#jvms-4.8)), derivation throws a `ClassFormatError`.

    - Otherwise, if the purported representation is not of a supported major or minor version ([§4.1](ch04-class-file-format.md#jvms-4.1)), derivation throws an `UnsupportedClassVersionError`.
...
3.  If C has a direct superclass, the symbolic reference from C to its direct superclass is resolved using the algorithm of [§5.4.3.1](ch05-loading-linking-initializing.md#jvms-5.4.3.1), with `L` acting as the defining loader of C. Note that if C is an interface it must have `Object` as its direct superclass, which must already have been loaded. Only `Object` has no direct superclass.
    ...
    - Otherwise, if the class named as the direct superclass of C has a `PermittedSubclasses` attribute ([§4.7.31](ch04-class-file-format.md#jvms-4.7.31)) and any of the following is true, derivation throws an `IncompatibleClassChangeError`:


      - The superclass is in a different run-time module than C ([§5.3.6](ch05-loading-linking-initializing.md#jvms-5.3.6)).

      - C does not have its `ACC_PUBLIC` flag set ([§4.1](ch04-class-file-format.md#jvms-4.1)) and the superclass is in a different run-time package than C ([§5.3](ch05-loading-linking-initializing.md#jvms-5.3)).
    ...
    - Otherwise, if C is a class and some instance method declared in C can override ([§5.4.5](ch05-loading-linking-initializing.md#jvms-5.4.5)) a `final` instance method declared in a superclass of C, derivation throws an `IncompatibleClassChangeError`.

    

4.  If C has any direct superinterfaces, the symbolic references from C to its direct superinterfaces are resolved using the algorithm of [§5.4.3.1](ch05-loading-linking-initializing.md#jvms-5.4.3.1), with `L` acting as the defining loader of C.
    ...
    - Otherwise, for each direct superinterface named by C, if the superinterface has a `PermittedSubclasses` attribute ([§4.7.31](ch04-class-file-format.md#jvms-4.7.31)) and any of the following is true, derivation throws an `IncompatibleClassChangeError`:


      - The superinterface is in a different run-time module than C.

      - C does not have its `ACC_PUBLIC` flag set ([§4.1](ch04-class-file-format.md#jvms-4.1)) and the superinterface is in a different run-time package than C.
...
If no exception is thrown in steps 1-4, then derivation of the class or interface C succeeds. The Java Virtual Machine marks C to have `L` as its defining loader, records that `L` is an initiating loader of C ([§5.3.4](ch05-loading-linking-initializing.md#jvms-5.3.4)), and creates C in the method area ([§2.5.4](ch02-jvm-structure.md#jvms-2.5.4)).

When derivation succeeds, the process of loading and creating C is not complete until every class loader that was involved in loading C (directly or indirectly) returns C as its result. Depending on the behavior of user-defined class loaders, the process of loading and creating C may yet fail ([§5.3.2](ch05-loading-linking-initializing.md#jvms-5.3.2)).
...
*Verification* ([§4.10](ch04-class-file-format.md#jvms-4.10)) ensures that the binary representation of a class or interface is structurally correct ([§4.9](ch04-class-file-format.md#jvms-4.9)). Verification may cause additional classes and interfaces to be loaded ([§5.3](ch05-loading-linking-initializing.md#jvms-5.3)) but need not cause them to be verified or prepared.

If the binary representation of a class or interface does not satisfy the static or structural constraints listed in [§4.9](ch04-class-file-format.md#jvms-4.9), then a `VerifyError` must be thrown at the point in the program that caused the class or interface to be verified.
...
*Preparation* involves creating the static fields for a class or interface and initializing such fields to their default values ([§2.3](ch02-jvm-structure.md#jvms-2.3), [§2.4](ch02-jvm-structure.md#jvms-2.4)). This does not require the execution of any Java Virtual Machine code; explicit initializers for static fields are executed as part of initialization ([§5.5](ch05-loading-linking-initializing.md#jvms-5.5)), not preparation.

During preparation of a class or interface C, the Java Virtual Machine also imposes loading constraints ([§5.3.4](ch05-loading-linking-initializing.md#jvms-5.3.4)):


1.  Let `L`<sub>`1`</sub> be the defining loader of C. For each instance method `m` declared in C that can override ([§5.4.5](ch05-loading-linking-initializing.md#jvms-5.4.5)) an instance method declared in a superclass or superinterface D = `<``N`<sub>`2`</sub>, `L`<sub>`2`</sub>`>`, for each class or interface name `N` mentioned by the descriptor of `m` ([§4.3.3](ch04-class-file-format.md#jvms-4.3.3)), the Java Virtual Machine imposes the loading constraint `N`<sup>`L`<sub>`1`</sub></sup> = `N`<sup>`L`<sub>`2`</sub></sup>.

2.  For each instance method `m` declared in a superinterface I = `<``N`<sub>`3`</sub>, `L`<sub>`3`</sub>`>` of C, if C does not itself declare an instance method that can override `m`, then a method is selected ([§5.4.6](ch05-loading-linking-initializing.md#jvms-5.4.6)) with respect to C and the method `m` in I. Let D = `<``N`<sub>`2`</sub>, `L`<sub>`2`</sub>`>` be the class or interface that declares the selected method. For each class or interface name `N` mentioned by the descriptor of `m`, the Java Virtual Machine imposes the loading constraint `N`<sup>`L`<sub>`2`</sub></sup> = `N`<sup>`L`<sub>`3`</sub></sup>.
...
Resolution of an unresolved symbolic reference to (i) a class or interface, (ii) a field, (iii) a method, (iv) a method type, (v) a method handle, or (vi) a dynamically-computed constant, proceeds in accordance with the rules given in [§5.4.3.1](ch05-loading-linking-initializing.md#jvms-5.4.3.1) through [§5.4.3.5](ch05-loading-linking-initializing.md#jvms-5.4.3.5). In the first three of those sections, the class or interface in whose run-time constant pool the symbolic reference appears is labeled D. Then:
...
Because errors occurring on an initial attempt at resolution are thrown again on subsequent attempts, a class in one module that attempts to access, via resolution of a symbolic reference in its run-time constant pool, an unexported `public` type in a different module will always receive the same error indicating an inaccessible type ([§5.4.4](ch05-loading-linking-initializing.md#jvms-5.4.4)), *even if the Java SE Platform API is used to dynamically export the `public` type's package at some time after the class's first attempt*.

Resolution of an unresolved symbolic reference to a dynamically-computed call site proceeds in accordance with the rules given in [§5.4.3.6](ch05-loading-linking-initializing.md#jvms-5.4.3.6). Then:
...
1.  The defining loader of D is used to load and thereby create a class or interface denoted by `N`. This class or interface is C. The details of the process are given in [§5.3](ch05-loading-linking-initializing.md#jvms-5.3).
...
2.  If C is an array class and its element type is a `reference` type, then a symbolic reference to the class or interface representing the element type is resolved by invoking the algorithm in [§5.4.3.1](ch05-loading-linking-initializing.md#jvms-5.4.3.1) recursively.

3.  Finally, access control is applied for the access from D to C ([§5.4.4](ch05-loading-linking-initializing.md#jvms-5.4.4)).
...
- Otherwise, field lookup succeeded. Access control is applied for the access from D to the field which is the result of field lookup ([§5.4.4](ch05-loading-linking-initializing.md#jvms-5.4.4)). Then:
  ...
  - Otherwise, access control succeeded. Loading constraints are imposed, as follows.
    ...
    Let `<`E, `L`<sub>`1`</sub>`>` be the class or interface in which the referenced field is actually declared. Let `L`<sub>`2`</sub> be the defining loader of D.

    For any class or interface name `N` mentioned by the descriptor of the referenced field ([§4.3.2](ch04-class-file-format.md#jvms-4.3.2)), the Java Virtual Machine imposes the loading constraint `N`<sup>`L`<sub>`1`</sub></sup> = `N`<sup>`L`<sub>`2`</sub></sup> ([§5.3.4](ch05-loading-linking-initializing.md#jvms-5.3.4)).
...
2.  Otherwise, method resolution attempts to locate the referenced method in C and its superclasses:
    ...
    - If C declares exactly one method with the name specified by the method reference, and the declaration is a signature polymorphic method ([§2.9.3](ch02-jvm-structure.md#jvms-2.9.3)), then method lookup succeeds. The descriptor specified by the method reference is resolved, as if by resolution of an unresolved symbolic reference to a method type ([§5.4.3.5](ch05-loading-linking-initializing.md#jvms-5.4.3.5)).
...
- Otherwise, method lookup succeeded. Access control is applied for the access from D to the method which is the result of method lookup ([§5.4.4](ch05-loading-linking-initializing.md#jvms-5.4.4)). Then:
  ...
  - Otherwise, access control succeeded. Loading constraints are imposed, as follows.
    ...
    Let `<`E, `L`<sub>`1`</sub>`>` be the class or interface in which the referenced method `m` is actually declared. Let `L`<sub>`2`</sub> be the defining loader of D.

    For each class or interface name `N` mentioned by the descriptor of the referenced method ([§4.3.3](ch04-class-file-format.md#jvms-4.3.3)), the Java Virtual Machine imposes the loading constraint `N`<sup>`L`<sub>`1`</sub></sup> = `N`<sup>`L`<sub>`2`</sub></sup> ([§5.3.4](ch05-loading-linking-initializing.md#jvms-5.3.4)).
...
4.  Otherwise, if the maximally-specific superinterface methods ([§5.4.3.3](ch05-loading-linking-initializing.md#jvms-5.4.3.3)) of C for the name and descriptor specified by the method reference include exactly one method that does not have its `ACC_ABSTRACT` flag set, then this method is chosen and method lookup succeeds.
...
- Otherwise, method lookup succeeded. Access control is applied for the access from D to the method which is the result of method lookup ([§5.4.4](ch05-loading-linking-initializing.md#jvms-5.4.4)). Then:
  ...
  - Otherwise, access control succeeded. Loading constraints are imposed, as follows.
    ...
    Let `<`E, `L`<sub>`1`</sub>`>` be the class or interface in which the referenced interface method `m` is actually declared. Let `L`<sub>`2`</sub> be the defining loader of D.
...
To resolve an unresolved symbolic reference to a method type, it is as if resolution occurs of unresolved symbolic references to the classes and interfaces ([§5.4.3.1](ch05-loading-linking-initializing.md#jvms-5.4.3.1)) whose names are mentioned by the method descriptor ([§4.3.3](ch04-class-file-format.md#jvms-4.3.3)), in the order in which they are mentioned.
...
Resolution of an unresolved symbolic reference to a method handle is more complicated. Each method handle resolved by the Java Virtual Machine has an equivalent instruction sequence called its *bytecode behavior*, indicated by the method handle's *kind*. The integer values and descriptions of the nine kinds of method handle are given in [Table 5.4.3.5-A](ch05-loading-linking-initializing.md#jvms-5.4.3.5-220).

Symbolic references by an instruction sequence to fields or methods are indicated by `C.x:T`, where `x` and `T` are the name and descriptor ([§4.3.2](ch04-class-file-format.md#jvms-4.3.2), [§4.3.3](ch04-class-file-format.md#jvms-4.3.3)) of the field or method, and `C` is the class or interface in which the field or method is to be found.
...
Let `MH` be the symbolic reference to a method handle ([§5.1](ch05-loading-linking-initializing.md#jvms-5.1)) being resolved. Also:
...
1.  R is resolved. This occurs as if by field resolution ([§5.4.3.2](ch05-loading-linking-initializing.md#jvms-5.4.3.2)) when `MH`'s bytecode behavior is kind 1, 2, 3, or 4, and as if by method resolution ([§5.4.3.3](ch05-loading-linking-initializing.md#jvms-5.4.3.3)) when `MH`'s bytecode behavior is kind 5, 6, 7, or 8, and as if by interface method resolution ([§5.4.3.4](ch05-loading-linking-initializing.md#jvms-5.4.3.4)) when `MH`'s bytecode behavior is kind 9.
...
3.  A reference to an instance of `java.lang.invoke.MethodType` is obtained as if by resolution of an unresolved symbolic reference to a method type that contains the method descriptor specified in [Table 5.4.3.5-B](ch05-loading-linking-initializing.md#jvms-5.4.3.5-250) for the kind of `MH`.

    It is as if the symbolic reference to a method handle contains a symbolic reference to the method type that the resolved method handle will eventually have. The detailed structure of the method type is obtained by inspecting [Table 5.4.3.5-B](ch05-loading-linking-initializing.md#jvms-5.4.3.5-250).
...
If the method referenced by R has the `ACC_VARARGS` flag set ([§4.6](ch04-class-file-format.md#jvms-4.6)), then the `java.lang.invoke.MethodHandle` instance is a variable arity method handle; otherwise, it is a fixed arity method handle.
...
1.  R gives a symbolic reference to a *bootstrap method handle*. The bootstrap method handle is resolved ([§5.4.3.5](ch05-loading-linking-initializing.md#jvms-5.4.3.5)) to obtain a `reference` to an instance of `java.lang.invoke.MethodHandle`.
...
3.  If R is a symbolic reference to a dynamically-computed call site, then it gives a method descriptor.
    ...
    A `reference` to an instance of `java.lang.invoke.MethodType` is obtained, as if by resolution of an unresolved symbolic reference to a method type ([§5.4.3.5](ch05-loading-linking-initializing.md#jvms-5.4.3.5)) with the same parameter and return types as the method descriptor.
...
Access control is applied during resolution ([§5.4.3](ch05-loading-linking-initializing.md#jvms-5.4.3)) to ensure that a reference to a class, interface, field, or method is permitted. Access control succeeds if a specified class, interface, field, or method is *accessible* to the referring class or interface.

A class or interface C is accessible to a class or interface D if and only if one of the following is true:


- C is `public`, and a member of the same run-time module as D ([§5.3.6](ch05-loading-linking-initializing.md#jvms-5.3.6)).
...
- R is `protected` and is declared in a class C, and D is either a subclass of C or C itself.
  ...
  During verification of D, it was required that, even if T is a superclass of D, the target reference of a `protected` field access or method invocation must be an instance of D or a subclass of D ([§4.10.1.8](ch04-class-file-format.md#jvms-4.10.1.8)).
...
- Otherwise, `M` has a `NestHost` attribute, and its `host_class_index` item is used as an index into the run-time constant pool of `M`. The symbolic reference at that index is resolved ([§5.4.3.1](ch05-loading-linking-initializing.md#jvms-5.4.3.1)).
...
An instance method `m`<sub>`C`</sub> *can override* another instance method `m`<sub>`A`</sub> iff all of the following are true:


- `m`<sub>`C`</sub> has the same name and descriptor as `m`<sub>`A`</sub>.
...
- One of the following is true:
  ...
  - `m`<sub>`A`</sub> is marked neither `ACC_PUBLIC` nor `ACC_PROTECTED` nor `ACC_PRIVATE`, and either (a) the declaration of `m`<sub>`A`</sub> appears in the same run-time package as the declaration of `m`<sub>`C`</sub>, or (b) if `m`<sub>`A`</sub> is declared in a class A and `m`<sub>`C`</sub> is declared in a class C, then there exists a method `m`<sub>`B`</sub> declared in a class B such that C is a subclass of B and B is a subclass of A and `m`<sub>`C`</sub> can override `m`<sub>`B`</sub> and `m`<sub>`B`</sub> can override `m`<sub>`A`</sub>.
...
During execution of an *invokeinterface* or *invokevirtual* instruction, a method is *selected* with respect to (i) the run-time type of the object on the stack, and (ii) a method that was previously *resolved* by the instruction. The rules to select a method with respect to a class or interface C and a method `m`<sub>`R`</sub> are as follows:


1.  If `m`<sub>`R`</sub> is marked `ACC_PRIVATE`, then it is the selected method.

2.  Otherwise, the selected method is determined by the following lookup procedure:


    - If C contains a declaration of an instance method `m` that can override `m`<sub>`R`</sub> ([§5.4.5](ch05-loading-linking-initializing.md#jvms-5.4.5)), then `m` is the selected method.

    - Otherwise, if C has a superclass, a search for a declaration of an instance method that can override `m`<sub>`R`</sub> is performed, starting with the direct superclass of C and continuing with the direct superclass of that class, and so forth, until a method is found or no further superclasses exist. If a method is found, it is the selected method.

    - Otherwise, the maximally-specific superinterface methods of C are determined ([§5.4.3.3](ch05-loading-linking-initializing.md#jvms-5.4.3.3)). If exactly one matches `m`<sub>`R`</sub>'s name and descriptor and is not `abstract`, then it is the selected method.

      Any maximally-specific superinterface method selected in this step can override `m`<sub>`R`</sub>; there is no need to check this explicitly.

    


While C will typically be a class, it may be an interface when these rules are applied during preparation ([§5.4.2](ch05-loading-linking-initializing.md#jvms-5.4.2)).


## 5.5. Initialization


*Initialization* of a class or interface involves assigning any `ConstantValue` attribute values to its `static` fields and executing any declared class or interface initialization method ([§2.9.2](ch02-jvm-structure.md#jvms-2.9.2)).

A class or interface C may be initialized only as a result of:


- The execution of any one of the Java Virtual Machine instructions *new*, *getstatic*, *putstatic*, or *invokestatic* that references C ([§new](ch06-instruction-set.md#jvms-6.5.new), [§getstatic](ch06-instruction-set.md#jvms-6.5.getstatic), [§putstatic](ch06-instruction-set.md#jvms-6.5.putstatic), [§invokestatic](ch06-instruction-set.md#jvms-6.5.invokestatic)).
...
- The first invocation of a `java.lang.invoke.MethodHandle` instance which was the result of method handle resolution ([§5.4.3.5](ch05-loading-linking-initializing.md#jvms-5.4.3.5)) for a method handle of kind 2 (`REF_getStatic`), 4 (`REF_putStatic`), 6 (`REF_invokeStatic`), or 8 (`REF_newInvokeSpecial`).

  This implies that the class of a bootstrap method is initialized when the bootstrap method is invoked for an *invokedynamic* instruction ([§invokedynamic](ch06-instruction-set.md#jvms-6.5.invokedynamic)), as part of the continuing resolution of the call site specifier.

- Invocation of certain reflective methods in the class library ([§2.12](ch02-jvm-structure.md#jvms-2.12)), for example, in class `Class` or in package `java.lang.reflect`.
...
- Its designation as the initial class or interface at Java Virtual Machine startup ([§5.2](ch05-loading-linking-initializing.md#jvms-5.2)).
...
6.  Otherwise, record the fact that initialization of C is in progress by the current thread, and release `LC`.
    ...
    Then, initialize each `static` field of C with the constant value in its `ConstantValue` attribute ([§4.7.2](ch04-class-file-format.md#jvms-4.7.2)), in the order the fields appear in the `ClassFile` structure.

7.  Next, if C is a class rather than an interface, then let SC be its superclass and let SI<sub>1</sub>, ..., SI<sub>n</sub> be all superinterfaces of C (whether direct or indirect) that declare at least one non-`abstract`, non-`static` method. The order of superinterfaces is given by a recursive enumeration over the superinterface hierarchy of each interface directly implemented by C. For each interface I directly implemented by C (in the order of the `interfaces` array of C), the enumeration recurs on I's superinterfaces (in the order of the `interfaces` array of I) before returning I.

    For each S in the list \[ SC, SI<sub>1</sub>, ..., SI<sub>n</sub> \], if S has not yet been initialized, then recursively perform this entire procedure for S. If necessary, verify and prepare S first.
...
The Java Virtual Machine executes code in threads ([§2.5](ch02-jvm-structure.md#jvms-2.5)). A thread is either a non-daemon thread, a daemon thread, or a shutdown hook.
...
A thread *terminates* if either (i) its `run` method completes normally, or (ii) its `run` method completes abruptly and the relevant uncaught exception handler ([§2.10](ch02-jvm-structure.md#jvms-2.10)) completes normally or abruptly. With no code left to run, the thread has completed execution and therefore has no current method ([§2.5.1](ch02-jvm-structure.md#jvms-2.5.1)).
