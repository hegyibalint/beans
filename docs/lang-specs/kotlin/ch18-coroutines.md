# Chapter 18. Asynchronous Programming with Coroutines


## Contents

[18.1. Suspending functions](#kls-18.1)

[18.2. Coroutines](#kls-18.2)

[18.3. Implementation details](#kls-18.3)

[18.3.1. kotlin.coroutines.Continuation\<T\>](#kls-18.3.1)

[18.3.2. Continuation Passing Style](#kls-18.3.2)

[18.3.3. Coroutine state machine](#kls-18.3.3)

[18.3.4. Continuation interception](#kls-18.3.4)

[18.3.5. Coroutine intrinsics](#kls-18.3.5)
...
In Kotlin, coroutines are used to implement [suspending functions](#kls-18.1) and can switch contexts only at suspension points.
...
`CoroutineContext` represents the context of the continuation and is an indexed set from `CoroutineContext.Key` to `CoroutineContext.Element` (e.g., a special kind of map). It is used to store coroutine-local information, and takes important part in [Continuation interception](#kls-18.3.4).
...
Each suspendable function goes through a transformation from normally invoked function to continuation passing style (CPS). For a suspendable function with parameters *p*<sub>1</sub>, *p*<sub>2</sub>, ..., *p*<sub>N</sub> and result type *T* a new function is generated, with an additional parameter *p*<sub>N+1</sub> of type `kotlin.coroutines.Continuation<T>` and return type changed to `kotlin.Any?`. The calling convention for such function is different from regular functions as a suspendable function may either *suspend* or *return*.
...
- Access the coroutine's continuation object by calling `suspendCoroutineUninterceptedOrReturn` intrinsic or any of its wrappers;
