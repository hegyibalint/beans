...
[5.1. Classifier type inheritance](ch05-inheritance.md#kls-5.1)

[5.1.1. Abstract classes](ch05-inheritance.md#kls-5.1.1)

[5.1.2. Sealed classes and interfaces](ch05-inheritance.md#kls-5.1.2)

[5.1.3. Inheritance from built-in types](ch05-inheritance.md#kls-5.1.3)

[5.2. Matching and subsumption of declarations](ch05-inheritance.md#kls-5.2)

[5.3. Inheriting](ch05-inheritance.md#kls-5.3)

[5.4. Overriding](ch05-inheritance.md#kls-5.4)
...
A class or object type is allowed to inherit from only one class type (called its **direct superclass**) and multiple interface types. As specified in the [declaration section](ch04-declarations.md#kls-4.1), if the superclass of a class or object type is not specified, it is assumed to be `kotlin.Any`. This means, among other things, that every class or object type always has a direct superclass.
...
A [data class](ch04-declarations.md#kls-4.1), [enum class](ch04-declarations.md#kls-4.1) or [annotation class](ch04-declarations.md#kls-4.1) cannot be declared `open` or `abstract`, i.e., are always closed and cannot be inherited from. Declaring a class `sealed` also implicitly declares it `abstract`.
...
Inheritance is the primary mechanism of introducing subtyping relations between user-defined types in Kotlin. When a classifier type *A* is declared with base types *B*<sub>1</sub>, ..., *B*<sub>m</sub>, it introduces subtyping relations *A* <: *B*<sub>1</sub>, ..., *A* <: *B*<sub>m</sub>, which are then used in overload resolution and type inference mechanisms.
...
A class declared `abstract` cannot be instantiated, i.e., an object of this class cannot be created directly. Abstract classes are implicitly `open` and their primary purpose is to be inherited from. Abstract classes (similarly to [interfaces](ch04-declarations.md#kls-4.1)) allow for `abstract` [property](ch04-declarations.md#kls-4.3) and [function](ch04-declarations.md#kls-4.2) declarations in their scope.
...
A class or interface (but not a [functional interface](ch04-declarations.md#kls-4.1)) may be declared `sealed`, making it special from the inheritance point-of-view.

- A `sealed` class is implicitly `abstract` (and these two modifiers are exclusive);
- A `sealed` class or interface can only be inherited from by types declared in the same package and in the same [module](ch04-declarations.md#kls-4.1), and which have a fully-qualified name (meaning local and anonymous types cannon be inherited from `sealed` types);
- `Sealed` classes and interfaces allow for exhaustiveness checking of [when expressions](ch08-expressions.md#kls-8) for values of such types. Any sealed type `S` is associated with its *direct non-sealed subtypes*: a set of non-sealed types, which are either direct subtypes of `S` or transitive subtypes of `S` via some number of other `sealed` types. These direct non-sealed subtypes form the boundary for exhaustiveness checks.
...
The notions of matching and subsumption are used when talking about how declarations are [inherited](#kls-5.3) and [overridden](#kls-5.4).
...
A callable declaration (that is, a [property](ch04-declarations.md#kls-4.3) or [member function](ch04-declarations.md#kls-4.2) declaration) inside a classifier declaration is said to be *inheritable* if:

- Its visibility (and the visibility of its getter and setter, if present) is not `private`.

If the declaration *B* of the base classifier type is inheritable, no other inheritable declaration from the base classifier types subsume *B*, no declarations in the derived classifier type [override](#kls-5.4) *B*, then *B* is *inherited* by the derived classifier type.
...
A callable declaration (that is, a [property](ch04-declarations.md#kls-4.3) or [member function](ch04-declarations.md#kls-4.2) declaration) inside a classifier declaration is said to be *overridable* if:
