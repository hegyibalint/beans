...
[1.1. Organization of the Specification](ch01-introduction.md#jls-1.1)

[1.2. Example Programs](ch01-introduction.md#jls-1.2)

[1.3. Notation](ch01-introduction.md#jls-1.3)

[1.4. Relationship to Predefined Classes and Interfaces](ch01-introduction.md#jls-1.4)

[1.5. Preview Features](ch01-introduction.md#jls-1.5)

[1.5.1. Restrictions on the Use of Preview Features](ch01-introduction.md#jls-1.5.1)

[1.5.2. Current Preview Language Features](ch01-introduction.md#jls-1.5.2)

[1.6. Feedback](ch01-introduction.md#jls-1.6)

[1.7. References](ch01-introduction.md#jls-1.7)
...
The Java programming language is normally compiled to the bytecode instruction set and binary format defined in *The Java Virtual Machine Specification, Java SE 26 Edition*.
...
The type system of the Java programming language occasionally relies on the notion of a *substitution*. The notation `[``F`<sub>`1`</sub>`:=``T`<sub>`1`</sub>`,...,``F`<sub>`n`</sub>`:=``T`<sub>`n`</sub>`]` denotes substitution of F<sub>i</sub> by T<sub>i</sub> for 1 ≤ *i* ≤ *n*.
...
- If preview features are enabled, then any source code reference to a class or interface declared using a preview language feature causes a *preview warning*, unless one of the following is true:
  ...
  - The reference appears in a declaration that is annotated to suppress preview warnings ([§9.6.4.5](ch09-interfaces.md#jls-9.6.4.5)).

  - The reference appears in an import declaration ([§7.5](ch07-packages-modules.md#jls-7.5)).
...
Java SE 26 defines one preview language feature:
...
Unicode Consortium, The. *The Unicode Standard, Version 17.0.0*. South San Francisco: The Unicode Consortium, 2025. ISBN 978-1-936213-35-1.
