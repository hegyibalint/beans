...
[5.1. Kinds of Conversion](ch05-conversions-contexts.md#jls-5.1)

[5.1.1. Identity Conversion](ch05-conversions-contexts.md#jls-5.1.1)

[5.1.2. Widening Primitive Conversion](ch05-conversions-contexts.md#jls-5.1.2)

[5.1.3. Narrowing Primitive Conversion](ch05-conversions-contexts.md#jls-5.1.3)

[5.1.4. Widening and Narrowing Primitive Conversion](ch05-conversions-contexts.md#jls-5.1.4)

[5.1.5. Widening Reference Conversion](ch05-conversions-contexts.md#jls-5.1.5)

[5.1.6. Narrowing Reference Conversion](ch05-conversions-contexts.md#jls-5.1.6)

[5.1.6.1. Allowed Narrowing Reference Conversion](ch05-conversions-contexts.md#jls-5.1.6.1)

[5.1.6.2. Checked and Unchecked Narrowing Reference Conversions](ch05-conversions-contexts.md#jls-5.1.6.2)

[5.1.6.3. Narrowing Reference Conversions at Run Time](ch05-conversions-contexts.md#jls-5.1.6.3)

[5.1.7. Boxing Conversion](ch05-conversions-contexts.md#jls-5.1.7)

[5.1.8. Unboxing Conversion](ch05-conversions-contexts.md#jls-5.1.8)

[5.1.9. Unchecked Conversion](ch05-conversions-contexts.md#jls-5.1.9)

[5.1.10. Capture Conversion](ch05-conversions-contexts.md#jls-5.1.10)

[5.1.11. String Conversion](ch05-conversions-contexts.md#jls-5.1.11)

[5.1.12. Forbidden Conversions](ch05-conversions-contexts.md#jls-5.1.12)

[5.2. Assignment Contexts](ch05-conversions-contexts.md#jls-5.2)

[5.3. Invocation Contexts](ch05-conversions-contexts.md#jls-5.3)

[5.4. String Contexts](ch05-conversions-contexts.md#jls-5.4)

[5.5. Casting Contexts](ch05-conversions-contexts.md#jls-5.5)

[5.6. Numeric Contexts](ch05-conversions-contexts.md#jls-5.6)

[5.7. Testing Contexts](ch05-conversions-contexts.md#jls-5.7)


Every expression written in the Java programming language either produces no result ([§15.1](ch15-expressions.md#jls-15.1)) or has a type that can be deduced at compile time ([§15.3](ch15-expressions.md#jls-15.3)). When an expression appears in most contexts, it must be *compatible* with a type expected in that context; this type is called the *target type*. For convenience, compatibility of an expression with its surrounding context is facilitated in two ways:


- First, for some expressions, termed *poly expressions* ([§15.2](ch15-expressions.md#jls-15.2)), the deduced type can be influenced by the target type. The same expression can have different types in different contexts.
...
Similarly, the rules determining whether a target type allows an implicit conversion vary depending on the kind of context, the type of the expression, and, in one special case, the value of a constant expression ([§15.29](ch15-expressions.md#jls-15.29)). A conversion from type S to type T allows an expression of type S to be treated at compile time as if it had type T instead. In some cases this will require a corresponding action at run time to check the validity of the conversion or to translate the run-time value of the expression into a form appropriate for the new type T.
...
- Assignment contexts ([§5.2](ch05-conversions-contexts.md#jls-5.2), [§15.26](ch15-expressions.md#jls-15.26)), in which an expression's value is assigned to a named variable. Primitive and reference types are subject to widening, values may be boxed or unboxed, and some primitive constant expressions may be subject to narrowing. An unchecked conversion may also occur.

- Strict invocation contexts ([§5.3](ch05-conversions-contexts.md#jls-5.3), [§15.9](ch15-expressions.md#jls-15.9), [§15.12](ch15-expressions.md#jls-15.12)), in which an argument is assigned to a formal parameter of a constructor or method. Widening primitive, widening reference, and unchecked conversions may occur.

- Loose invocation contexts ([§5.3](ch05-conversions-contexts.md#jls-5.3), [§15.9](ch15-expressions.md#jls-15.9), [§15.12](ch15-expressions.md#jls-15.12)), in which, like strict invocation contexts, an argument is assigned to a formal parameter. Method or constructor invocations may provide this context if no applicable declaration can be found using only strict invocation contexts. In addition to widening and unchecked conversions, this context allows boxing and unboxing conversions to occur.

- String contexts ([§5.4](ch05-conversions-contexts.md#jls-5.4), [§15.18.1](ch15-expressions.md#jls-15.18.1)), in which a value of any type is converted to an object of type `String`.

- Casting contexts ([§5.5](ch05-conversions-contexts.md#jls-5.5)), in which an expression's value is converted to a type explicitly specified by a cast operator ([§15.16](ch15-expressions.md#jls-15.16)). Casting contexts are more inclusive than assignment or loose invocation contexts, allowing any specific conversion other than a string conversion, but certain casts to a reference type are checked for correctness at run time.

- Numeric contexts ([§5.6](ch05-conversions-contexts.md#jls-5.6)), in which the operands of a numeric operator or some other expressions that operate on numbers may be widened to a common type.

- Testing contexts ([§5.7](ch05-conversions-contexts.md#jls-5.7)), in which an expression's value is converted to a type explicitly specified by a pattern ([§14.30](ch14-blocks-statements-patterns.md#jls-14.30)). Testing contexts are more inclusive than assignment or loose invocation contexts, but not as inclusive as casting contexts.


The term "conversion" is also used to describe, without being specific, any conversions allowed in a particular context. For example, we say that an expression that is the initializer of a local variable is subject to "assignment conversion", meaning that a specific conversion will be implicitly chosen for that expression according to the rules for the assignment context. As another example, we say that an expression undergoes "casting conversion" to mean that the expression's value will be converted as permitted in a casting context.
...
A widening primitive conversion from `int` to `float`, or from `long` to `float`, or from `long` to `double`, may result in *loss of precision*, that is, the result may lose some of the least significant bits of the value. In this case, the resulting floating-point value will be a correctly rounded version of the integer value, using the round to nearest rounding policy ([§15.4](ch15-expressions.md#jls-15.4)).
...
Despite the fact that loss of precision may occur, a widening primitive conversion never results in a run-time exception ([§11.1.1](ch11-exceptions.md#jls-11.1.1)).
...
1.  In the first step, the floating-point number is converted either to a `long`, if T is `long`, or to an `int`, if T is `byte`, `short`, `char`, or `int`, as follows:
    ...
    - If the floating-point number is NaN ([§4.2.3](ch04-types-values-variables.md#jls-4.2.3)), the result of the first step of the conversion is an `int` or `long` `0`.

    - Otherwise, if the floating-point number is not an infinity, the floating-point value is rounded to an integer value `V` using the round toward zero rounding policy ([§4.2.4](ch04-types-values-variables.md#jls-4.2.4)). Then there are two cases:
...
2.  In the second step:
    ...
    - If T is `byte`, `char`, or `short`, the result of the conversion is the result of a narrowing conversion to type T ([§5.1.3](ch05-conversions-contexts.md#jls-5.1.3)) of the result of the first step.

    


A narrowing conversion from `double` to `float` occurs as determined by the rules of IEEE 754 for converting between binary floating-point formats, using the round to nearest rounding policy ([§15.4](ch15-expressions.md#jls-15.4)). This conversion can lose precision, but also lose range, resulting in a `float` zero from a nonzero `double` and a `float` infinity from a finite `double`. A `double` NaN is converted to a `float` NaN and a `double` infinity is converted to the same-signed `float` infinity.

Despite the fact that overflow, underflow, or other loss of information may occur, a narrowing primitive conversion never results in a run-time exception ([§11.1.1](ch11-exceptions.md#jls-11.1.1)).
...
First, the `byte` is converted to an `int` via widening primitive conversion ([§5.1.2](ch05-conversions-contexts.md#jls-5.1.2)), and then the resulting `int` is converted to a `char` by narrowing primitive conversion ([§5.1.3](ch05-conversions-contexts.md#jls-5.1.3)).


### 5.1.5. Widening Reference Conversion


A *widening reference conversion* exists from any reference type S to any reference type T, provided S is a subtype of T ([§4.10](ch04-types-values-variables.md#jls-4.10)).
...
The null type is not a reference type ([§4.1](ch04-types-values-variables.md#jls-4.1)), and so a widening reference conversion does not exist from the null type to a reference type. However, many conversion contexts explicitly allow the null type to be converted to a reference type.


### 5.1.6. Narrowing Reference Conversion


A *narrowing reference conversion* treats expressions of a reference type S as expressions of a different reference type T, where S is not a subtype of T. The supported pairs of types are defined in [§5.1.6.1](ch05-conversions-contexts.md#jls-5.1.6.1). Unlike widening reference conversion, the types need not be directly related. However, there are restrictions that prohibit conversion between certain pairs of types when it can be statically proven that no value can be of both types.

A narrowing reference conversion may require a test at run time to validate that a value of type S is a legitimate value of type T. However, due to the lack of parameterized type information at run time, some conversions cannot be fully validated by a run time test; they are flagged at compile time ([§5.1.6.2](ch05-conversions-contexts.md#jls-5.1.6.2)). For conversions that can be fully validated by a run time test, and for certain conversions that involve parameterized type information but can still be partially validated at run time, a `ClassCastException` is thrown if the test fails ([§5.1.6.3](ch05-conversions-contexts.md#jls-5.1.6.3)).
...
- S is not a subtype of T ([§4.10](ch04-types-values-variables.md#jls-4.10))

- If there exists a parameterized type X that is a supertype of T, and a parameterized type Y that is a supertype of S, such that the erasures of X and Y are the same, then X and Y are not provably distinct ([§4.5](ch04-types-values-variables.md#jls-4.5)).
...
- One of the following cases applies:
  ...
  1.  S is a class or interface type, and T is a class or interface type, and S names a class or interface that is not disjoint from the class or interface named by T. ("disjoint" is defined below.)

  2.  S is the class type `Object` or the interface type `java.io.Serializable` or `Cloneable` (the only interfaces implemented by arrays ([§10.8](ch10-arrays.md#jls-10.8))), and T is an array type.
  ...
  6.  S is an intersection type S<sub>1</sub> `&` ... `&` S<sub>n</sub>, and for all *i* (1 ≤ *i* ≤ *n*), either a widening reference conversion or a narrowing reference conversion exists from S<sub>i</sub> to T.

  7.  T is an intersection type T<sub>1</sub> `&` ... `&` T<sub>n</sub>, and for all *i* (1 ≤ *i* ≤ *n*), either a widening reference conversion or a narrowing reference conversion exists from S to T<sub>i</sub>.
...
- A class named C is disjoint from an interface named I if (i) it is not the case that C `<:` I, and (ii) one of the following cases applies:
  ...
  - C is freely extensible ([§8.1.1.2](ch08-classes.md#jls-8.1.1.2)), and I is `sealed`, and C is disjoint from all of the permitted direct subclasses and subinterfaces of I.
...
The final clause above implies that two freely extensible interfaces ([§9.1.1.4](ch09-interfaces.md#jls-9.1.1.4)) are not disjoint.
...
If a narrowing reference conversion is unchecked, then the Java Virtual Machine will not be able to fully validate its type correctness, possibly leading to heap pollution ([§4.12.2](ch04-types-values-variables.md#jls-4.12.2)). To flag this to the programmer, an unchecked narrowing reference conversion causes a compile-time *unchecked warning*, unless suppressed by `@SuppressWarnings` ([§9.6.4.5](ch09-interfaces.md#jls-9.6.4.5)). Conversely, if a narrowing reference conversion is not unchecked, then it is checked; the Java Virtual Machine will be able to fully validate its type correctness, so no warning is given at compile time.
...
- A narrowing reference conversion from a type S to an intersection type T<sub>1</sub> `&` ... `&` T<sub>n</sub> is unchecked if there exists a T<sub>i</sub> (1 ≤ *i* ≤ *n*) such that S is not a subtype of T<sub>i</sub> and a narrowing reference conversion from S to T<sub>i</sub> is unchecked.
...
- An unchecked narrowing reference conversion from S to an intersection type T<sub>1</sub> `&` ... `&` T<sub>n</sub> is completely unchecked if, for all *i* (1 ≤ *i* ≤ *n*), either S `<:` T<sub>i</sub> or a narrowing reference conversion from S to T<sub>i</sub> is completely unchecked.
...
- Otherwise, let R be the class of the object referred to by the value, and let T be the erasure ([§4.6](ch04-types-values-variables.md#jls-4.6)) of the type being converted to. Then:


  - If R is an ordinary class (not an array class):


    - If T is a class type, then R must be either the same class as T ([§4.3.4](ch04-types-values-variables.md#jls-4.3.4)) or a subclass of T, or a `ClassCastException` is thrown.

    - If T is an interface type, then R must implement interface T ([§8.1.5](ch08-classes.md#jls-8.1.5)), or a `ClassCastException` is thrown.
  ...
  - If R is an interface:
    ...
    - If T is a class type, then T must be `Object` ([§4.3.2](ch04-types-values-variables.md#jls-4.3.2)), or a `ClassCastException` is thrown.
...
If the conversion is to an intersection type T<sub>1</sub> `&` ... `&` T<sub>n</sub>, then for all *i* (1 ≤ *i* ≤ *n*), any run-time check required for a conversion from S to T<sub>i</sub> is also required for the conversion to the intersection type.
...
- From the null type to the null type
  ...
  This rule is necessary because the conditional operator ([§15.25](ch15-expressions.md#jls-15.25)) applies boxing conversion to the types of its operands, and uses the result in further calculations.
...
- If `p` is a value of any other type, boxing conversion is equivalent to an identity conversion ([§5.1.1](ch05-conversions-contexts.md#jls-5.1.1)).


If the value `p` being boxed is the result of evaluating a constant expression ([§15.29](ch15-expressions.md#jls-15.29)) of type `boolean`, `byte`, `char`, `short`, `int`, or `long`, and the result is `true`, `false`, a character in the range `'\u0000'` to `'\u007f'` inclusive, or an integer in the range `-128` to `127` inclusive, then let `a` and `b` be the results of any two boxing conversions of `p`. It is always the case that `a` `==` `b`.
...
A type is said to be *convertible to a numeric type* if it is a numeric type ([§4.2](ch04-types-values-variables.md#jls-4.2)), or it is a reference type that may be converted to a numeric type by unboxing conversion.
...
There is an *unchecked conversion* from the raw class or interface type ([§4.8](ch04-types-values-variables.md#jls-4.8)) G to any parameterized type of the form G`<`T<sub>1</sub>,...,T<sub>n</sub>`>`.

There is an *unchecked conversion* from the raw array type G`[]`<sup>k</sup> to any array type of the form G`<`T<sub>1</sub>,...,T<sub>n</sub>`>``[]`<sup>k</sup>. (The notation `[]`<sup>k</sup> indicates an array type of *k* dimensions.)

Use of an unchecked conversion causes a compile-time *unchecked warning* unless all type arguments T<sub>i</sub> (1 ≤ *i* ≤ *n*) are unbounded wildcards ([§4.5.1](ch04-types-values-variables.md#jls-4.5.1)), or the warning is suppressed by `@SuppressWarnings` ([§9.6.4.5](ch09-interfaces.md#jls-9.6.4.5)).
...
Let G name a generic type declaration ([§8.1.2](ch08-classes.md#jls-8.1.2), [§9.1.2](ch09-interfaces.md#jls-9.1.2)) with *n* type parameters A<sub>1</sub>,...,A<sub>n</sub> with corresponding bounds U<sub>1</sub>,...,U<sub>n</sub>.

There exists a *capture conversion* from a parameterized type G`<`T<sub>1</sub>,...,T<sub>n</sub>`>` ([§4.5](ch04-types-values-variables.md#jls-4.5)) to a parameterized type G`<`S<sub>1</sub>,...,S<sub>n</sub>`>`, where, for 1 ≤ *i* ≤ *n* :


- If T<sub>i</sub> is a wildcard type argument ([§4.5.1](ch04-types-values-variables.md#jls-4.5.1)) of the form `?`, then S<sub>i</sub> is a fresh type variable whose upper bound is U<sub>i</sub>`[``A`<sub>`1`</sub>`:=``S`<sub>`1`</sub>`,...,``A`<sub>`n`</sub>`:=``S`<sub>`n`</sub>`]` and whose lower bound is the null type ([§4.1](ch04-types-values-variables.md#jls-4.1)).

- If T<sub>i</sub> is a wildcard type argument of the form `?` `extends` B<sub>i</sub>, then S<sub>i</sub> is a fresh type variable whose upper bound is glb(B<sub>i</sub>, U<sub>i</sub>`[``A`<sub>`1`</sub>`:=``S`<sub>`1`</sub>`,...,``A`<sub>`n`</sub>`:=``S`<sub>`n`</sub>`]`) and whose lower bound is the null type.

  glb(V<sub>1</sub>,...,V<sub>m</sub>) is defined as V<sub>1</sub> `&` ... `&` V<sub>m</sub>.
...
- Otherwise, S<sub>i</sub> = T<sub>i</sub>.


Capture conversion on any type other than a parameterized type ([§4.5](ch04-types-values-variables.md#jls-4.5)) acts as an identity conversion ([§5.1.1](ch05-conversions-contexts.md#jls-5.1.1)).
...
A value `x` of primitive type T is first converted to a reference value as if by giving it as an argument to an appropriate method invocation:


- If T is `boolean`, then use `Boolean.valueOf(``x``)`.

- If T is `char`, then use `Character.valueOf(``x``)`.

- If T is `byte`, `short`, or `int`, then use `Integer.valueOf(``x``)`.

- If T is `long`, then use `Long.valueOf(``x``)`.

- If T is `float`, then use `Float.valueOf(``x``)`.

- If T is `double`, then use `Double.valueOf(``x``)`.
...
- Otherwise, the conversion is performed as if by an invocation of the `toString` method of the referenced object with no arguments; but if the result of invoking the `toString` method is `null`, then the string "`null`" is used instead.
  ...
  The `toString` method is defined by the primordial class `Object` ([§4.3.2](ch04-types-values-variables.md#jls-4.3.2)). Many classes override it, notably `Boolean`, `Character`, `Integer`, `Long`, `Float`, `Double`, and `String`.
...
*Assignment contexts* allow the value of an expression to be assigned ([§15.26](ch15-expressions.md#jls-15.26)) to a variable; the type of the expression must be converted to the type of the variable.

Assignment contexts allow the use of one of the following:


- an identity conversion ([§5.1.1](ch05-conversions-contexts.md#jls-5.1.1))

- a widening primitive conversion ([§5.1.2](ch05-conversions-contexts.md#jls-5.1.2))

- a widening reference conversion ([§5.1.5](ch05-conversions-contexts.md#jls-5.1.5))
...
- a boxing conversion ([§5.1.7](ch05-conversions-contexts.md#jls-5.1.7))

- a boxing conversion followed by a widening reference conversion

- an unboxing conversion ([§5.1.8](ch05-conversions-contexts.md#jls-5.1.8))

- an unboxing conversion followed by a widening primitive conversion


If, after the conversions listed above have been applied, the resulting type is a raw type ([§4.8](ch04-types-values-variables.md#jls-4.8)), an unchecked conversion ([§5.1.9](ch05-conversions-contexts.md#jls-5.1.9)) may then be applied.

In addition, if the expression is a constant expression ([§15.29](ch15-expressions.md#jls-15.29)) of type `byte`, `short`, `char`, or `int`:
...
It is a compile-time error if the chain of conversions contains two parameterized types that are not in the subtype relation ([§4.10](ch04-types-values-variables.md#jls-4.10)).
...
- A `ClassCastException` if, after the conversions above have been applied, the resulting value is an object which is not an instance of a subclass or subinterface of the erasure ([§4.6](ch04-types-values-variables.md#jls-4.6)) of the type of the variable.

  This circumstance can only arise as a result of heap pollution ([§4.12.2](ch04-types-values-variables.md#jls-4.12.2)). In practice, implementations need only perform casts when accessing a field or method of an object of parameterized type when the erased type of the field, or the erased return type of the method, differ from its unerased type.
...
- An `ArrayStoreException` in special cases involving array elements or field access ([§10.5](ch10-arrays.md#jls-10.5), [§15.26.1](ch15-expressions.md#jls-15.26.1)).
...
*Invocation contexts* allow an argument value in a method or constructor invocation ([§8.8.7.1](ch08-classes.md#jls-8.8.7.1), [§15.9](ch15-expressions.md#jls-15.9), [§15.12](ch15-expressions.md#jls-15.12)) to be assigned to a corresponding formal parameter.
...
If, after the conversions listed for an invocation context have been applied, the resulting type is a raw type ([§4.8](ch04-types-values-variables.md#jls-4.8)), an unchecked conversion ([§5.1.9](ch05-conversions-contexts.md#jls-5.1.9)) may then be applied.
...
- A `ClassCastException` if, after the type conversions above have been applied, the resulting value is an object which is not an instance of a subclass or subinterface of the erasure ([§4.6](ch04-types-values-variables.md#jls-4.6)) of the corresponding formal parameter type.
...
The target type in these contexts is always `String`, and a string conversion ([§5.1.11](ch05-conversions-contexts.md#jls-5.1.11)) of the non-`String` operand always occurs. Evaluation of the `+` operator then proceeds as specified in [§15.18.1](ch15-expressions.md#jls-15.18.1).


## 5.5. Casting Contexts


*Casting contexts* allow the operand of a cast expression ([§15.16](ch15-expressions.md#jls-15.16)) to be converted to the type explicitly specified by the cast operator. Compared to assignment contexts and invocation contexts, casting contexts allow the use of more of the conversions defined in [§5.1](ch05-conversions-contexts.md#jls-5.1), and allow more combinations of those conversions.
...
- a narrowing primitive conversion ([§5.1.3](ch05-conversions-contexts.md#jls-5.1.3))

- a widening and narrowing primitive conversion ([§5.1.4](ch05-conversions-contexts.md#jls-5.1.4))

- a boxing conversion ([§5.1.7](ch05-conversions-contexts.md#jls-5.1.7))

- a boxing conversion followed by a widening reference conversion ([§5.1.5](ch05-conversions-contexts.md#jls-5.1.5))
...
- a narrowing reference conversion ([§5.1.6](ch05-conversions-contexts.md#jls-5.1.6))
...
If a casting context makes use of a narrowing reference conversion that is checked or partially unchecked ([§5.1.6.2](ch05-conversions-contexts.md#jls-5.1.6.2), [§5.1.6.3](ch05-conversions-contexts.md#jls-5.1.6.3)), then a run time check will be performed on the class of the expression's value, possibly causing a `ClassCastException`. Otherwise, no run time check is performed.
...
- ≈ signifies identity conversion ([§5.1.1](ch05-conversions-contexts.md#jls-5.1.1))

- ω signifies widening primitive conversion ([§5.1.2](ch05-conversions-contexts.md#jls-5.1.2))

- η signifies narrowing primitive conversion ([§5.1.3](ch05-conversions-contexts.md#jls-5.1.3))

- ωη signifies widening and narrowing primitive conversion ([§5.1.4](ch05-conversions-contexts.md#jls-5.1.4))

- ⇑ signifies widening reference conversion ([§5.1.5](ch05-conversions-contexts.md#jls-5.1.5))

- ⇓ signifies narrowing reference conversion ([§5.1.6](ch05-conversions-contexts.md#jls-5.1.6))

- ⊕ signifies boxing conversion ([§5.1.7](ch05-conversions-contexts.md#jls-5.1.7))

- ⊗ signifies unboxing conversion ([§5.1.8](ch05-conversions-contexts.md#jls-5.1.8))
...
- The operand of a unary plus operator `+`, unary minus operator `-`, or bitwise complement operator `~` ([§15.15.3](ch15-expressions.md#jls-15.15.3), [§15.15.4](ch15-expressions.md#jls-15.15.4), [§15.15.5](ch15-expressions.md#jls-15.15.5))

- An operand of a multiplicative operator `*`, `/`, or `%` ([§15.17](ch15-expressions.md#jls-15.17))

- An operand of an addition or subtraction operator for numeric types `+` or `-` ([§15.18.2](ch15-expressions.md#jls-15.18.2))

- An operand of a shift operator `<<`, `>>`, or `>>>` ([§15.19](ch15-expressions.md#jls-15.19)). Operands of these shift operators are treated separately rather than as a group. A `long` shift distance (right operand) does not promote the value being shifted (left operand) to `long`.

- An operand of a numerical comparison operator `<`, `<=`, `>`, or `>=` ([§15.20.1](ch15-expressions.md#jls-15.20.1))

- An operand of a numerical equality operator `==` or `!=` ([§15.21.1](ch15-expressions.md#jls-15.21.1))

- An operand of an integer bitwise operator `&`, `^`, or `|` ([§15.22.1](ch15-expressions.md#jls-15.22.1))


An expression appears in a *numeric array context* if the expression is one of the following:


- A dimension expression in an array creation expression ([§15.10.1](ch15-expressions.md#jls-15.10.1))

- The index expression in an array access expression ([§15.10.3](ch15-expressions.md#jls-15.10.3))


An expression appears in a *numeric choice context* if the expression is one of the following:


- The second or third operand of a numeric conditional expression ([§15.25.2](ch15-expressions.md#jls-15.25.2))

- A result expression of a standalone `switch` expression ([§15.28.1](ch15-expressions.md#jls-15.28.1)) where all the result expressions are convertible to a numeric type
...
1.  If any expression is of a reference type, it is subjected to unboxing conversion ([§5.1.8](ch05-conversions-contexts.md#jls-5.1.8)).

2.  Next, widening primitive conversion ([§5.1.2](ch05-conversions-contexts.md#jls-5.1.2)) and narrowing primitive conversion ([§5.1.3](ch05-conversions-contexts.md#jls-5.1.3)) are applied to some expressions, according to the following rules:
    ...
    - Otherwise, none of the expressions are of type `double`, `float`, or `long`. In this case, the kind of context determines how the promoted type is chosen.
      ...
      - If any expression is of type `int` and is not a constant expression ([§15.29](ch15-expressions.md#jls-15.29)), then the promoted type is `int`, and other expressions that are not of type `int` undergo widening primitive conversion to `int`.
...
*Testing contexts* allow the operand of a pattern match operator ([§15.20.2](ch15-expressions.md#jls-15.20.2)), or the selector expression of a `switch` expression or statement that has at least one pattern `case` label associated with its switch block ([§14.11.1](ch14-blocks-statements-patterns.md#jls-14.11.1)) to be converted to a type as part of the process of pattern matching. As pattern matching is an inherently conditional process ([§14.30.2](ch14-blocks-statements-patterns.md#jls-14.30.2)), it is expected that a testing context will make use of conversions that may fail at run time.

If the expression is of a primitive type, then a testing context allows the use of an identity conversion ([§5.1.1](ch05-conversions-contexts.md#jls-5.1.1)).
...
- a narrowing reference conversion that is checked ([§5.1.6.2](ch05-conversions-contexts.md#jls-5.1.6.2))

  Unlike casting contexts, testing contexts do not permit narrowing reference conversions that are unchecked.
