...
[17.1. Synchronization](ch17-threads-locks.md#jls-17.1)

[17.2. Wait Sets and Notification](ch17-threads-locks.md#jls-17.2)

[17.2.1. Wait](ch17-threads-locks.md#jls-17.2.1)

[17.2.2. Notification](ch17-threads-locks.md#jls-17.2.2)

[17.2.3. Interruptions](ch17-threads-locks.md#jls-17.2.3)

[17.2.4. Interactions of Waits, Notification, and Interruption](ch17-threads-locks.md#jls-17.2.4)

[17.3. Sleep and Yield](ch17-threads-locks.md#jls-17.3)

[17.4. Memory Model](ch17-threads-locks.md#jls-17.4)

[17.4.1. Shared Variables](ch17-threads-locks.md#jls-17.4.1)

[17.4.2. Actions](ch17-threads-locks.md#jls-17.4.2)

[17.4.3. Programs and Program Order](ch17-threads-locks.md#jls-17.4.3)

[17.4.4. Synchronization Order](ch17-threads-locks.md#jls-17.4.4)

[17.4.5. Happens-before Order](ch17-threads-locks.md#jls-17.4.5)

[17.4.6. Executions](ch17-threads-locks.md#jls-17.4.6)

[17.4.7. Well-Formed Executions](ch17-threads-locks.md#jls-17.4.7)

[17.4.8. Executions and Causality Requirements](ch17-threads-locks.md#jls-17.4.8)

[17.4.9. Observable Behavior and Nonterminating Executions](ch17-threads-locks.md#jls-17.4.9)

[17.5. `final` Field Semantics](ch17-threads-locks.md#jls-17.5)

[17.5.1. Semantics of `final` Fields](ch17-threads-locks.md#jls-17.5.1)

[17.5.2. Reading `final` Fields During Construction](ch17-threads-locks.md#jls-17.5.2)

[17.5.3. Subsequent Modification of `final` Fields](ch17-threads-locks.md#jls-17.5.3)

[17.5.4. Write-Protected Fields](ch17-threads-locks.md#jls-17.5.4)

[17.6. Word Tearing](ch17-threads-locks.md#jls-17.6)

[17.7. Non-Atomic Treatment of `double` and `long`](ch17-threads-locks.md#jls-17.7)
...
The `synchronized` statement ([§14.19](ch14-blocks-statements-patterns.md#jls-14.19)) computes a reference to an object; it then attempts to perform a lock action on that object's monitor and does not proceed further until the lock action has successfully completed. After the lock action has been performed, the body of the `synchronized` statement is executed. If execution of the body is ever completed, either normally or abruptly, an unlock action is automatically performed on that same monitor.
...
Consider, for example, the example program traces shown in [Table 17.4-A](ch17-threads-locks.md#jls-17.4-A). This program uses local variables `r1` and `r2` and shared variables `A` and `B`. Initially, `A == B == 0`.
...
This situation is an example of a *data race* ([§17.4.5](ch17-threads-locks.md#jls-17.4.5)). When code contains a data race, counterintuitive results are often possible.
...
Another example of surprising results can be seen in [Table 17.4-C](ch17-threads-locks.md#jls-17.4-C). Initially, `p == q` and `p.x == 0`. This program is also incorrectly synchronized; it writes to shared memory without enforcing any ordering between those writes.
...
One common compiler optimization involves having the value read for `r2` reused for `r5`: they are both reads of `r1.x` with no intervening write. This situation is shown in [Table 17.4-D](ch17-threads-locks.md#jls-17.4-D).
...
This section provides the specification of the Java programming language memory model except for issues dealing with `final` fields, which are described in [§17.5](ch17-threads-locks.md#jls-17.5).
...
Local variables ([§14.4](ch14-blocks-statements-patterns.md#jls-14.4)), formal method parameters ([§8.4.1](ch08-classes.md#jls-8.4.1)), and exception handler parameters ([§14.20](ch14-blocks-statements-patterns.md#jls-14.20)) are never shared between threads and are unaffected by the memory model.
...
- *Synchronization actions*, which are:
  ...
  - Actions that start a thread or detect that a thread has terminated ([§17.4.4](ch17-threads-locks.md#jls-17.4.4)).
...
- *Thread divergence actions* ([§17.4.9](ch17-threads-locks.md#jls-17.4.9)). A thread divergence action is only performed by a thread that is in an infinite loop in which no memory, synchronization, or external actions are performed. If a thread performs a thread divergence action, it will be followed by an infinite number of thread divergence actions.
...
In non-terminating executions, not all external actions are observable. Non-terminating executions and observable actions are discussed in [§17.4.9](ch17-threads-locks.md#jls-17.4.9).
...
If we were to use sequential consistency as our memory model, many of the compiler and processor optimizations that we have discussed would be illegal. For example, in the trace in [Table 17.4-C](ch17-threads-locks.md#jls-17.4-C), as soon as the write of `3` to `p.x` occurred, subsequent reads of that location would be required to see that value.


### 17.4.4. Synchronization Order


Every execution has a *synchronization order*. A synchronization order is a total order over all of the synchronization actions of an execution. For each thread *t*, the synchronization order of the synchronization actions ([§17.4.2](ch17-threads-locks.md#jls-17.4.2)) in *t* is consistent with the program order ([§17.4.3](ch17-threads-locks.md#jls-17.4.3)) of *t*.
...
- A write to a volatile variable *v* ([§8.3.1.4](ch08-classes.md#jls-8.3.1.4)) *synchronizes-with* all subsequent reads of *v* by any thread (where "subsequent" is defined according to the synchronization order).
...
- There is a *happens-before* edge from the end of a constructor of an object to the start of a finalizer ([§12.6](ch12-execution.md#jls-12.6)) for that object.
...
The `wait` methods of class `Object` ([§17.2.1](ch17-threads-locks.md#jls-17.2.1)) have lock and unlock actions associated with them; their *happens-before* relationships are defined by these associated actions.
...
- A write to a `volatile` field ([§8.3.1.4](ch08-classes.md#jls-8.3.1.4)) *happens-before* every subsequent read of that field.
...
When a program contains two conflicting accesses ([§17.4.1](ch17-threads-locks.md#jls-17.4.1)) that are not ordered by a happens-before relationship, it is said to contain a *data race*.

The semantics of operations other than inter-thread actions, such as reads of array lengths ([§10.7](ch10-arrays.md#jls-10.7)), executions of checked casts ([§5.5](ch05-conversions-contexts.md#jls-5.5), [§15.16](ch15-expressions.md#jls-15.16)), and invocations of virtual methods ([§15.12](ch15-expressions.md#jls-15.12)), are not directly affected by data races.
...
If a program is correctly synchronized, then all executions of the program will appear to be sequentially consistent ([§17.4.3](ch17-threads-locks.md#jls-17.4.3)).
...
For the trace in [Table 17.4.5-A](ch17-threads-locks.md#jls-17.4.5-A), initially `A == B == 0`. The trace can observe `r2 == 0` and `r1 == 0` and still be *happens-before consistent*, since there are execution orders that allow each read to see the appropriate write.
...
Note that the synchronizes-with and happens-before elements are uniquely determined by the other components of an execution and the rules for well-formed executions ([§17.4.7](ch17-threads-locks.md#jls-17.4.7)).

An execution is *happens-before consistent* if its set of actions is *happens-before consistent* ([§17.4.5](ch17-threads-locks.md#jls-17.4.5)).
...
4.  The execution is *happens-before consistent* ([§17.4.6](ch17-threads-locks.md#jls-17.4.6)).
...
We use *f*\|<sub>*d*</sub> to denote the function given by restricting the domain of *f* to *d*. For all *x* in *d*, *f*\|<sub>*d*</sub>(*x*) = *f*(*x*), and for all *x* not in *d*, *f*\|<sub>*d*</sub>(*x*) is undefined.

We use *p*\|<sub>*d*</sub> to represent the restriction of the partial order *p* to the elements in *d*. For all *x*,*y* in *d*, *p*(*x*,*y*) if and only if *p*\|<sub>*d*</sub>(*x*,*y*). If either *x* or *y* are not in *d*, then it is not the case that *p*\|<sub>*d*</sub>(*x*,*y*).
...
Starting with the empty set as *C<sub>0</sub>*, we perform a sequence of steps where we take actions from the set of actions *A* and add them to a set of committed actions *C<sub>i</sub>* to get a new set of committed actions *C<sub>i+1</sub>*. To demonstrate that this is reasonable, for each *C<sub>i</sub>* we need to demonstrate an execution *E* containing *C<sub>i</sub>* that meets certain conditions.
...
- Sets of actions *C<sub>0</sub>*, *C<sub>1</sub>*, ... such that:
  ...
  - *C<sub>0</sub>* is the empty set

  - *C<sub>i</sub>* is a proper subset of *C<sub>i+1</sub>*

  - *A* = ∪ (*C<sub>0</sub>*, *C<sub>1</sub>*, ...)

  

  If *A* is finite, then the sequence *C<sub>0</sub>*, *C<sub>1</sub>*, ... will be finite, ending in a set *C<sub>n</sub>* = *A*.

  If *A* is infinite, then the sequence *C<sub>0</sub>*, *C<sub>1</sub>*, ... may be infinite, and it must be the case that the union of all elements of this infinite sequence is equal to *A*.

- Well-formed executions *E<sub>1</sub>*, ..., where *E<sub>i</sub>* = \< *P, A<sub>i</sub>, po<sub>i</sub>, so<sub>i</sub>, W<sub>i</sub>, V<sub>i</sub>, sw<sub>i</sub>, hb<sub>i</sub>* \>.


Given these sets of actions *C<sub>0</sub>*, ... and executions *E<sub>1</sub>*, ... , every action in *C<sub>i</sub>* must be one of the actions in *E<sub>i</sub>*. All actions in *C<sub>i</sub>* must share the same relative happens-before order and synchronization order in both *E<sub>i</sub>* and *E*. Formally:


1.  *C<sub>i</sub>* is a subset of *A<sub>i</sub>*

2.  *hb<sub>i</sub>*\|<sub>*C<sub>i</sub>*</sub> = *hb*\|<sub>*C<sub>i</sub>*</sub>

3.  *so<sub>i</sub>*\|<sub>*C<sub>i</sub>*</sub> = *so*\|<sub>*C<sub>i</sub>*</sub>


The values written by the writes in *C<sub>i</sub>* must be the same in both *E<sub>i</sub>* and *E*. Only the reads in *C<sub>i-1</sub>* need to see the same writes in *E<sub>i</sub>* as in *E*. Formally:


4.  *V<sub>i</sub>*\|<sub>*C<sub>i</sub>*</sub> = *V*\|<sub>*C<sub>i</sub>*</sub>

5.  *W<sub>i</sub>*\|<sub>*C<sub>i-1</sub>*</sub> = *W*\|<sub>*C<sub>i-1</sub>*</sub>


All reads in *E<sub>i</sub>* that are not in *C<sub>i-1</sub>* must see writes that happen-before them. Each read *r* in *C<sub>i</sub>* - *C<sub>i-1</sub>* must see writes in *C<sub>i-1</sub>* in both *E<sub>i</sub>* and *E*, but may see a different write in *E<sub>i</sub>* from the one it sees in *E*. Formally:


6.  For any read *r* in *A<sub>i</sub>* - *C<sub>i-1</sub>*, we have *hb<sub>i</sub>(W<sub>i</sub>(r), r)*

7.  For any read *r* in (*C<sub>i</sub>* - *C<sub>i-1</sub>*), we have *W<sub>i</sub>(r)* in *C<sub>i-1</sub>* and *W(r)* in *C<sub>i-1</sub>*


Given a set of sufficient synchronizes-with edges for *E<sub>i</sub>*, if there is a release-acquire pair that happens-before ([§17.4.5](ch17-threads-locks.md#jls-17.4.5)) an action you are committing, then that pair must be present in all *E<sub>j</sub>*, where *j* ≥ *i*. Formally:


8.  Let *ssw<sub>i</sub>* be the *sw<sub>i</sub>* edges that are also in the transitive reduction of *hb<sub>i</sub>* but not in *po*. We call *ssw<sub>i</sub>* the *sufficient synchronizes-with edges for *E<sub>i</sub>**. If *ssw<sub>i</sub>(x, y)* and *hb<sub>i</sub>(y, z)* and *z* in *C<sub>i</sub>*, then *sw<sub>j</sub>(x, y)* for all *j* ≥ *i*.

    If an action *y* is committed, all external actions that happen-before *y* are also committed.

9.  If *y* is in *C<sub>i</sub>*, *x* is an external action and *hb<sub>i</sub>(x, y)*, then *x* in *C<sub>i</sub>*.


**Example 17.4.8-1. Happens-before Consistency Is Not Sufficient**


Happens-before consistency is a necessary, but not sufficient, set of constraints. Merely enforcing happens-before consistency would allow for unacceptable behaviors - those that violate the requirements we have established for programs. For example, happens-before consistency allows values to appear "out of thin air". This can be seen by a detailed examination of the trace in [Table 17.4.8-A](ch17-threads-locks.md#jls-17.4.8-A).
...
Obviously, some actions may be committed early and some may not. If, for example, one of the writes in [Table 17.4.8-A](ch17-threads-locks.md#jls-17.4.8-A) were committed before the read of that variable, the read could see the write, and the "out-of-thin-air" result could occur. Informally, we allow an action to be committed early if we know that the action can occur without assuming some data race occurs. In [Table 17.4.8-A](ch17-threads-locks.md#jls-17.4.8-A), we cannot perform either write early, because the writes cannot occur unless the reads see the result of a data race.
...
Given a write *w*, a freeze *f*, an action *a* (that is not a read of a `final` field), a read *r<sub>1</sub>* of the `final` field frozen by *f*, and a read *r<sub>2</sub>* such that *hb(w, f)*, *hb(f, a)*, *mc(a, r<sub>1</sub>)*, and *dereferences(r<sub>1</sub>, r<sub>2</sub>)*, then when determining which values can be seen by *r<sub>2</sub>*, we consider *hb(w, r<sub>2</sub>)*. (This *happens-before* ordering does not transitively close with other *happens-before* orderings.)

Note that the *dereferences* order is reflexive, and *r<sub>1</sub>* can be the same as *r<sub>2</sub>*.
...
Even then, there are a number of complications. If a `final` field is initialized to a constant expression ([§15.29](ch15-expressions.md#jls-15.29)) in the field declaration, changes to the `final` field may not be observed, since uses of that `final` field are replaced at compile time with the value of the constant expression.
