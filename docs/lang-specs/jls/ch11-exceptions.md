...
[11.1. The Kinds and Causes of Exceptions](ch11-exceptions.md#jls-11.1)

[11.1.1. The Kinds of Exceptions](ch11-exceptions.md#jls-11.1.1)

[11.1.2. The Causes of Exceptions](ch11-exceptions.md#jls-11.1.2)

[11.1.3. Asynchronous Exceptions](ch11-exceptions.md#jls-11.1.3)

[11.2. Compile-Time Checking of Exceptions](ch11-exceptions.md#jls-11.2)

[11.2.1. Exception Analysis of Expressions](ch11-exceptions.md#jls-11.2.1)

[11.2.2. Exception Analysis of Statements and Constructor Invocations](ch11-exceptions.md#jls-11.2.2)

[11.2.3. Exception Checking](ch11-exceptions.md#jls-11.2.3)

[11.3. Run-Time Handling of an Exception](ch11-exceptions.md#jls-11.3)
...
Programs can also throw exceptions explicitly, using `throw` statements ([§14.18](ch14-blocks-statements-patterns.md#jls-14.18)).
...
Every exception is represented by an instance of the class `Throwable` or one of its subclasses ([§11.1](ch11-exceptions.md#jls-11.1)). Such an object can be used to carry information from the point at which an exception occurs to the handler that catches it. Handlers are established by `catch` clauses of `try` statements ([§14.20](ch14-blocks-statements-patterns.md#jls-14.20)).

During the process of throwing an exception, the Java Virtual Machine abruptly completes, one by one, any expressions, statements, method and constructor invocations, initializers, and field initialization expressions that have begun but not completed execution in the current thread. This process continues until a handler is found that indicates that it handles that particular exception by naming the class of the exception or a superclass of the class of the exception ([§11.2](ch11-exceptions.md#jls-11.2)). If no such handler is found, then the exception may be handled by one of a hierarchy of uncaught exception handlers ([§11.3](ch11-exceptions.md#jls-11.3)) - thus every effort is made to avoid letting an exception go unhandled.

The exception mechanism of the Java SE Platform is integrated with its synchronization model ([§17.1](ch17-threads-locks.md#jls-17.1)), so that monitors are unlocked as `synchronized` statements ([§14.19](ch14-blocks-statements-patterns.md#jls-14.19)) and invocations of `synchronized` methods ([§8.4.3.6](ch08-classes.md#jls-8.4.3.6), [§15.12](ch15-expressions.md#jls-15.12)) complete abruptly.
...
Programs can use the pre-existing exception classes of the Java SE Platform API in `throw` statements, or define additional exception classes as subclasses of `Throwable` or of any of its subclasses, as appropriate. To take advantage of compile-time checking for exception handlers ([§11.2](ch11-exceptions.md#jls-11.2)), it is typical to define most new exception classes as checked exception classes, that is, as subclasses of `Exception` that are not subclasses of `RuntimeException`.

The class `Error` is a separate subclass of `Throwable`, distinct from `Exception` in the class hierarchy, to allow programs to use the idiom "`} catch (Exception e) {`" ([§11.2.3](ch11-exceptions.md#jls-11.2.3)) to catch all exceptions from which recovery may be possible without catching errors from which recovery is typically not possible.

Note that a subclass of `Throwable` cannot be generic ([§8.1.2](ch08-classes.md#jls-8.1.2)).
...
- A `throw` statement ([§14.18](ch14-blocks-statements-patterns.md#jls-14.18)) was executed.

- An enabled `assert` statement was executed, and evaluation of its boolean expression evaluated to `false` ([§14.10](ch14-blocks-statements-patterns.md#jls-14.10)).

- An abnormal execution condition was synchronously detected by the Java Virtual Machine, namely:


  - evaluation of an expression violates the normal semantics of the Java programming language ([§15.6](ch15-expressions.md#jls-15.6)), such as an integer divide by zero.

  - an error occurs while loading, linking, or initializing part of the program ([§12.2](ch12-execution.md#jls-12.2), [§12.3](ch12-execution.md#jls-12.3), [§12.4](ch12-execution.md#jls-12.4)); in this case, an instance of a subclass of `LinkageError` is thrown.
...
- An asynchronous exception occurred ([§11.1.3](ch11-exceptions.md#jls-11.1.3)).
...
Note that `StackOverflowError`, a subclass of `VirtualMachineError`, may be thrown synchronously by method invocation ([§15.12.4.5](ch15-expressions.md#jls-15.12.4.5)) as well as asynchronously due to `native` method execution or Java Virtual Machine resource limitations. Similarly, `OutOfMemoryError`, another subclass of `VirtualMachineError`, may be thrown synchronously during class instance creation ([§15.9.4](ch15-expressions.md#jls-15.9.4), [§12.5](ch12-execution.md#jls-12.5)), array creation ([§15.10.2](ch15-expressions.md#jls-15.10.2), [§10.6](ch10-arrays.md#jls-10.6)), class initialization ([§12.4.2](ch12-execution.md#jls-12.4.2)), and boxing conversion ([§5.1.7](ch05-conversions-contexts.md#jls-5.1.7)), as well as asynchronously.
...
We say that a statement or expression *can throw* an exception class E if, according to the rules in [§11.2.1](ch11-exceptions.md#jls-11.2.1) and [§11.2.2](ch11-exceptions.md#jls-11.2.2), the execution of the statement or expression can result in an exception of class E being thrown.

We say that a `catch` clause *can catch* its catchable exception class(es):


- The catchable exception class of a uni-`catch` clause is the declared type of its exception parameter ([§14.20](ch14-blocks-statements-patterns.md#jls-14.20)).
...
A class instance creation expression ([§15.9](ch15-expressions.md#jls-15.9)) can throw an exception class E iff either:
...
- E is one of the exception types of the invocation type of the chosen constructor ([§15.12.2.6](ch15-expressions.md#jls-15.12.2.6)); or
...
A method invocation expression ([§15.12](ch15-expressions.md#jls-15.12)) can throw an exception class E iff either:
...
- E is one of the exception types of the invocation type of the chosen method ([§15.12.2.6](ch15-expressions.md#jls-15.12.2.6)).


A lambda expression ([§15.27](ch15-expressions.md#jls-15.27)) can throw no exception classes.

A `switch` expression ([§15.28](ch15-expressions.md#jls-15.28)) can throw an exception class E iff either:
...
A `try` statement ([§14.20](ch14-blocks-statements-patterns.md#jls-14.20)) can throw an exception class E iff either:
...
A `switch` statement ([§14.11](ch14-blocks-statements-patterns.md#jls-14.11)) can throw an exception class E iff either:
...
A constructor invocation ([§8.8.7.1](ch08-classes.md#jls-8.8.7.1)) can throw an exception class E iff either:


- Some expression of the constructor invocation's parameter list can throw E; or

- E is determined to be an exception class of the `throws` clause of the constructor that is invoked ([§15.12.2.6](ch15-expressions.md#jls-15.12.2.6)).
...
It is a compile-time error if a class variable initializer ([§8.3.2](ch08-classes.md#jls-8.3.2)) or static initializer ([§8.7](ch08-classes.md#jls-8.7)) of a named class or interface *can throw* a checked exception class.

It is a compile-time error if an instance variable initializer ([§8.3.2](ch08-classes.md#jls-8.3.2)) or instance initializer ([§8.6](ch08-classes.md#jls-8.6)) of a named class *can throw* a checked exception class, unless the named class has at least one explicitly declared constructor and the exception class or one of its superclasses is explicitly declared in the `throws` clause of each constructor.

Note that no compile-time error is due if an instance variable initializer or instance initializer of an anonymous class ([§15.9.5](ch15-expressions.md#jls-15.9.5)) can throw an exception class. In a named class, it is the responsibility of the programmer to propagate information about which exception classes can be thrown by initializers, by declaring a suitable `throws` clause on any explicit constructor declaration. This relationship between the checked exception classes thrown by a class's initializers and the checked exception classes declared by a class's constructors is assured for an anonymous class declaration, because no explicit constructor declarations are possible and a Java compiler always generates a constructor with a suitable `throws` clause for the anonymous class declaration based on the checked exception classes that its initializers can throw.

It is a compile-time error if a `catch` clause *can catch* checked exception class E<sub>1</sub> and it is not the case that the `try` block corresponding to the `catch` clause *can throw* a checked exception class that is a subclass or superclass of E<sub>1</sub>, unless E<sub>1</sub> is `Exception` or a superclass of `Exception`.

It is a compile-time error if a `catch` clause *can catch* an exception class E<sub>1</sub> and a preceding `catch` clause of the immediately enclosing `try` statement *can catch* E<sub>1</sub> or a superclass of E<sub>1</sub>.

A Java compiler is encouraged to issue a warning if a `catch` clause can catch checked exception class E<sub>1</sub> and the `try` block corresponding to the `catch` clause can throw checked exception class E<sub>2</sub>, where E<sub>2</sub> `<:` E<sub>1</sub>, and a preceding `catch` clause of the immediately enclosing `try` statement can catch checked exception class E<sub>3</sub>, where E<sub>2</sub> `<:` E<sub>3</sub> `<:` E<sub>1</sub>.
...
By the rules above, each alternative in a multi-`catch` clause ([§14.20](ch14-blocks-statements-patterns.md#jls-14.20)) must be able to catch some exception class thrown by the `try` block and uncaught by previous `catch` clauses. For example, the second `catch` clause below would cause a compile-time error because exception analysis determines that `SubclassOfFoo` is already caught by the first `catch` clause:
...
When an exception is thrown ([§14.18](ch14-blocks-statements-patterns.md#jls-14.18)), control is transferred from the code that caused the exception to the nearest dynamically enclosing `catch` clause, if any, of a `try` statement ([§14.20](ch14-blocks-statements-patterns.md#jls-14.20)) that can handle the exception.
...
- If within a method, then the caller is the method invocation expression ([§15.12](ch15-expressions.md#jls-15.12)) that was executed to cause the method to be invoked.

- If within a constructor or an instance initializer or the initializer for an instance variable, then the caller is the class instance creation expression ([§15.9](ch15-expressions.md#jls-15.9)) or the method invocation of `newInstance` that was executed to cause an object to be created.

- If within a static initializer or an initializer for a `static` variable, then the caller is the expression that used the class or interface so as to cause it to be initialized ([§12.4](ch12-execution.md#jls-12.4)).
...
Equivalently, a `catch` clause will catch any exception object that is an `instanceof` ([§15.20.2](ch15-expressions.md#jls-15.20.2)) one of its catchable exception classes.

The control transfer that occurs when an exception is thrown causes abrupt completion of expressions ([§15.6](ch15-expressions.md#jls-15.6)) and statements ([§14.1](ch14-blocks-statements-patterns.md#jls-14.1)) until a `catch` clause is encountered that can handle the exception; execution then continues by executing the block of that `catch` clause. The code that caused the exception is never resumed.
...
In situations where it is desirable to ensure that one block of code is always executed after another, even if that other block of code completes abruptly, a `try` statement with a `finally` clause ([§14.20.2](ch14-blocks-statements-patterns.md#jls-14.20.2)) may be used.
...
The exact rules for abrupt completion and for the catching of exceptions are specified in detail with the specification of each statement in [§14 (Blocks, Statements, and Patterns)](ch14-blocks-statements-patterns.md) and for expressions in [§15 (Expressions)](ch15-expressions.md) (especially [§15.6](ch15-expressions.md#jls-15.6)).
...
The declaration of the method `thrower` must have a `throws` clause because it can throw instances of `TestException`, which is a checked exception class ([§11.1.1](ch11-exceptions.md#jls-11.1.1)). A compile-time error would occur if the `throws` clause were omitted.
