...
[10.1. Array Types](ch10-arrays.md#jls-10.1)

[10.2. Array Variables](ch10-arrays.md#jls-10.2)

[10.3. Array Creation](ch10-arrays.md#jls-10.3)

[10.4. Array Access](ch10-arrays.md#jls-10.4)

[10.5. Array Store Exception](ch10-arrays.md#jls-10.5)

[10.6. Array Initializers](ch10-arrays.md#jls-10.6)

[10.7. Array Members](ch10-arrays.md#jls-10.7)

[10.8. `Class` Objects for Arrays](ch10-arrays.md#jls-10.8)

[10.9. An Array of Characters Is Not a `String`](ch10-arrays.md#jls-10.9)


In the Java programming language, *arrays* are objects ([§4.3.1](ch04-types-values-variables.md#jls-4.3.1)), are dynamically created, and may be assigned to variables of type `Object` ([§4.3.2](ch04-types-values-variables.md#jls-4.3.2)). All methods of class `Object` may be invoked on an array.
...
Array types are used in declarations and in cast expressions ([§15.16](ch15-expressions.md#jls-15.16)).
...
Each bracket pair in an array type may be annotated by type annotations ([§9.7.4](ch09-interfaces.md#jls-9.7.4)). An annotation applies to the bracket pair (or ellipsis, in a variable arity parameter declaration) that follows it.
...
The supertypes of an array type are specified in [§4.10.3](ch04-types-values-variables.md#jls-4.10.3).

The supertype relation for array types is not the same as the superclass relation. The direct supertype of `Integer``[]` is `Number``[]` according to [§4.10.3](ch04-types-values-variables.md#jls-4.10.3), but the direct superclass of `Integer``[]` is `Object` according to the `Class` object for `Integer``[]` ([§10.8](ch10-arrays.md#jls-10.8)). This does not matter in practice, because `Object` is also a supertype of all array types.


## 10.2. Array Variables


A variable of array type holds a reference to an object. Declaring a variable of array type does not create an array object or allocate any space for array components. It creates only the variable itself, which can contain a reference to an array. However, the initializer part of a declarator ([§8.3](ch08-classes.md#jls-8.3), [§9.3](ch09-interfaces.md#jls-9.3), [§14.4.1](ch14-blocks-statements-patterns.md#jls-14.4.1)) may create an array, a reference to which then becomes the initial value of the variable.
...
The array type of a variable depends on the bracket pairs that may appear as part of the type at the beginning of a variable declaration, or as part of the declarator for the variable, or both. Specifically, in the declaration of a field, formal parameter, local variable, or record component ([§8.3](ch08-classes.md#jls-8.3), [§8.4.1](ch08-classes.md#jls-8.4.1), [§9.3](ch09-interfaces.md#jls-9.3), [§9.4](ch09-interfaces.md#jls-9.4), [§14.4.1](ch14-blocks-statements-patterns.md#jls-14.4.1), [§14.14.2](ch14-blocks-statements-patterns.md#jls-14.14.2), [§15.27.1](ch15-expressions.md#jls-15.27.1), [§8.10.1](ch08-classes.md#jls-8.10.1)), the array type of the variable is denoted by:
...
The return type of a method ([§8.4.5](ch08-classes.md#jls-8.4.5)) may be an array type. The precise array type depends on the bracket pairs that may appear as part of the type at the beginning of the method declaration, or after the method's formal parameter list, or both. The array type is denoted by:
...
If an array variable `v` has type A`[]`, where A is a reference type, then `v` can hold a reference to an instance of any array type B`[]`, provided B can be assigned to A ([§5.2](ch05-conversions-contexts.md#jls-5.2)). This may result in a run-time exception on a *later* assignment; see [§10.5](ch10-arrays.md#jls-10.5) for a discussion.


## 10.3. Array Creation


An array is created by an array creation expression ([§15.10.1](ch15-expressions.md#jls-15.10.1)) or an array initializer ([§10.6](ch10-arrays.md#jls-10.6)).
...
A component of an array is accessed by an array access expression ([§15.10.3](ch15-expressions.md#jls-15.10.3)) that consists of an expression whose value is an array reference followed by an indexing expression enclosed by `[` and `]`, as in `A[i]`.
...
The program declares a variable `ia` that has type array of `int`, that is, `int[]`. The variable `ia` is initialized to reference a newly created array object, created by an array creation expression ([§15.10.1](ch15-expressions.md#jls-15.10.1)). The array creation expression specifies that the array should have `101` components. The length of the array is available using the field `length`, as shown. The program fills the array with the integers from `0` to `100`, sums these integers, and prints the result.


  

Arrays must be indexed by `int` values; `short`, `byte`, or `char` values may also be used as index values because they are subjected to unary numeric promotion ([§5.6](ch05-conversions-contexts.md#jls-5.6)) and become `int` values.

An attempt to access an array component with a `long` index value results in a compile-time error.

All array accesses are checked at run time; an attempt to use an index that is less than zero or greater than or equal to the length of the array causes an `ArrayIndexOutOfBoundsException` to be thrown ([§15.10.4](ch15-expressions.md#jls-15.10.4)).
...
If the type of the value being assigned is not assignment-compatible ([§5.2](ch05-conversions-contexts.md#jls-5.2)) with the component type, an `ArrayStoreException` is thrown.

If the component type of an array were not reifiable ([§4.7](ch04-types-values-variables.md#jls-4.7)), the Java Virtual Machine could not perform the store check described in the preceding paragraph. This is why an array creation expression with a non-reifiable element type is forbidden ([§15.10.1](ch15-expressions.md#jls-15.10.1)). One may declare a variable of an array type whose element type is non-reifiable, but assignment of the result of an array creation expression to the variable will necessarily cause an unchecked warning ([§5.1.9](ch05-conversions-contexts.md#jls-5.1.9)).
...
An *array initializer* may be specified in a field declaration ([§8.3](ch08-classes.md#jls-8.3), [§9.3](ch09-interfaces.md#jls-9.3)) or local variable declaration ([§14.4](ch14-blocks-statements-patterns.md#jls-14.4)), or as part of an array creation expression ([§15.10.1](ch15-expressions.md#jls-15.10.1)), to create an array and provide some initial values.
...
The following production from [§8.3](ch08-classes.md#jls-8.3) is shown here for convenience:
...
- All the members inherited from class `Object`; the only method of `Object` that is not inherited is its `clone` method.
  ...
  See [§9.6.4.4](ch09-interfaces.md#jls-9.6.4.4) for another situation where the difference between `public` and non-`public` methods of `Object` requires special care.
...
Note that the cast to `T[]` in the code above would generate an unchecked warning ([§5.1.9](ch05-conversions-contexts.md#jls-5.1.9)) if arrays were really implemented this way.
