...
fn processed() -> Vec<(model::Source, model::Class)> {
...
fn a_module_descriptor_contributes_nothing() {
    let sources: Vec<model::Source> = processed().into_iter().map(|(source, _)| source).collect();

    assert!(!sources.iter().any(|source| matches!(
        source,
        model::Source::ClassFile { path } if path.ends_with("module-info.class")
    )));
