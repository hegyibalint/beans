...
impl Arguments {
    fn parse(arguments: impl Iterator<Item = OsString>) -> Result<Self, io::Error> {
        ...
        let path = path.unwrap_or_else(|| {
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("examples/inspection/inspect.java")
        });
        if !cst && !semantic {
            semantic = true;
        }
...
fn type_ref(reference: &Spanned<TypeRef>) -> String {
    match reference.value() {
        TypeRef::Named { segments } => segments
            .iter()
            .map(|segment| component(segment.value()))
            .collect::<Vec<_>>()
            .join("."),
