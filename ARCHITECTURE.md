...
| `beans` | The facade: the `NodePayload` union, the composed `Registries`, per-extension dispatch, the `Store` aggregate (graph + registries + interner), and the `Workspace` engine that owns indexing + resolution policy. Composes every language vertical unconditionally — no language Cargo features (ADR-0033). | `src/lib.rs`, `src/workspace.rs` |
